IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_rul_ApplicabilityAssociationFlagDetails')
DROP PROCEDURE [dbo].[st_rul_ApplicabilityAssociationFlagDetails]
GO
SET ANSI_NULLS ON
GO
...
Description: Fetch the flag values for AllEmployeeFlag, AllInsiderFlag, AllEmployeeInsiderFlag and 
			...
			If AllEmployeeFlag=1 then there will be only one record in rul_ApplicabilityMaster for combination MapToTypeCodeId-MapToId
			If AllInsiderFlag=1 then there will be only one record in rul_ApplicabilityMaster for combination MapToTypeCodeId-MapToId
			If AllEmployeeInsiderFlag = 1 then there can be more than 1 records in rul_ApplicabilityMaster for combination MapToTypeCodeId-MapToId
...
Gaurishankar	02-Feb-2016		Changes for All Employee Applicability to notify CO.
Gaurishankar	28-Apr-2016		Update to feach for All (Co, Non Employee and Corporate Employees) Applicability flag.
Usage:
EXEC st_rul_TradingPolicyDetails 1
-------------------------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[st_rul_ApplicabilityAssociationFlagDetails]
...
BEGIN
	...
	BEGIN TRY
		...
		IF(EXISTS(SELECT ApplicabilityId FROM rul_ApplicabilityMaster WHERE MapToTypeCodeId = @inp_nMapToTypeCodeId AND MapToId = @inp_nMapToId))
		BEGIN
			...
			INNER JOIN rul_ApplicabilityMaster AM
