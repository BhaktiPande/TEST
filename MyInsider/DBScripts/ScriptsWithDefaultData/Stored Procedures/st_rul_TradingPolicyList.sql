IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_rul_TradingPolicyList')
DROP PROCEDURE [dbo].[st_rul_TradingPolicyList]
GO
SET ANSI_NULLS ON
GO
...
Tushar			07-Jul-2015		Change query for From Date & To Date search parameters.

Usage:
EXEC 
-------------------------------------------------------------------------------------------------*/

CREATE PROCEDURE [dbo].[st_rul_TradingPolicyList]
...
BEGIN
	...
	DECLARE @ERR_TRADINGPOLICY_LIST INT = 15057 -- Error occurred while fetching trading policy list. (While fetching list of Current Version of Trading Policies) .
	...
	BEGIN TRY
		...
		IF @inp_sSortField = 'rul_grd_15052' -- Trading policy name
		BEGIN 
			SELECT @inp_sSortField = 'TP.TradingPolicyName  ' 
		END
		
		IF @inp_sSortField = 'rul_grd_15053' -- Applicable from date
		BEGIN 
			SELECT @inp_sSortField = 'TP.ApplicableFromDate ' 
		END		
		
		IF @inp_sSortField = 'rul_grd_15054' -- Applicable to date
		BEGIN 
			SELECT @inp_sSortField = 'TP.ApplicableToDate ' 
		END	
		
		IF @inp_sSortField = 'rul_grd_15055' -- Applicable To
		BEGIN 
			SELECT @inp_sSortField = 'CdTPFor.CodeName ' 
		END	
		
		IF @inp_sSortField = 'rul_grd_15056' -- Status
		BEGIN 
		...
		TP.TradingPolicyName AS rul_grd_15052 /*[Policy Name]*/,
		TP.ApplicableFromDate AS rul_grd_15053 /*[Applicable From Date]*/,
		TP.ApplicableToDate AS rul_grd_15054 /*[Applicable To Date]*/,
		CdTPFor.CodeName AS rul_grd_15055 /*[Applicable To]*/,
		CdTPStatus.CodeName AS rul_grd_15056 /*[Status]*/
