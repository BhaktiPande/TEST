IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_InitialDisclosureStatusForUser')
DROP PROCEDURE [st_tra_InitialDisclosureStatusForUser]
GO
...
CREATE PROCEDURE [dbo].[st_tra_InitialDisclosureStatusForUser] --516
	 @inp_iUserInfoID				INT
	 ,@inp_iUserTypeID				INT
...
BEGIN
	...
	BEGIN TRY
		...
		IF EXISTS (SELECT * FROM eve_EventLog e WHERE EventCodeId IN (153007) AND UserInfoId = @iParentID AND e.EventDate>=@dtDateofBecomingInsider)
		BEGIN
			SET @nAllDocumentsCompleteFlag = @nStatusFlag_Complete
		END
		...
		BEGIN
			IF EXISTS (SELECT * FROM eve_EventLog e WHERE EventCodeId IN (153007) AND UserInfoId = @inp_iUserInfoID AND e.EventDate>=@dtDateofBecomingInsider)
			BEGIN
				...
				SELECT @nTransactionMasterID = MapToId, @ntmpEventCode = EventCodeId, @dttmpEventDate = EventDate 
					FROM eve_EventLog WHERE EventCodeId = 153007 and UserInfoId = @inp_iUserInfoID AND EventDate>=@dtDateofBecomingInsider 
			END
			ELSE
			BEGIN
				...
				IF EXISTS (SELECT * FROM eve_EventLog e WHERE EventCodeId IN (153008) AND UserInfoId = @inp_iUserInfoID AND e.EventDate>=@dtDateofBecomingInsider)
				BEGIN
		...
		BEGIN			
			...
			BEGIN
				...
				IF EXISTS (SELECT * FROM eve_EventLog WHERE UserInfoId = @inp_iUserInfoID AND EventCodeId = 153009 AND EventDate>=@dtDateofBecomingInsider)
				BEGIN
