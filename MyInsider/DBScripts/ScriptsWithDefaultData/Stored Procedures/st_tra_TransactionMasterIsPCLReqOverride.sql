IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_TransactionMasterIsPCLReqOverride')
DROP PROCEDURE [dbo].[st_tra_TransactionMasterIsPCLReqOverride]
GO
SET ANSI_NULLS ON
GO
...
Arundhati		08-Oct-2015		Cursor not getting closed properly, if the procedure is executed in loop.
Tushar			05-Feb-2015		Query change for fetching limit from preclearance map to type. 

-------------------------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[st_tra_TransactionMasterIsPCLReqOverride]
...
BEGIN
	...
	--WHERE TransactionMasterId = @inp_nTransactionMasterId
		...
		BEGIN
			...
			IF EXISTS (SELECT * FROM rul_TradingPolicyForTransactionSecurity 
			...
			BEGIN
				...
				BEGIN
					...
					IF (@inp_UpdateTD = 1)
					BEGIN
						UPDATE tra_TransactionDetails SET IsPLCReq = 1 WHERE TransactionDetailsId = @nTransactionDetailsId
					END
					ELSE
					BEGIN
