...
BEGIN
	...
	CREATE TABLE #TEMP_REPORT_DATA(MassUploadHistoryId VARCHAR(200),UserInfoId VARCHAR(200), EmployeeId VARCHAR(200), EmployeeName VARCHAR(200), Designation VARCHAR(200), Grade VARCHAR(200), Location VARCHAR(200), Department VARCHAR(200), CompanyName VARCHAR(200), TypeofInsider VARCHAR(200), RelationWithEmployee VARCHAR(200), 
	...
	FROM 
		...
		LEFT JOIN #TEMP_DESIGNATION AS TDG ON TDG.EMPLOYEE_DESIGNATION = ISNULL(RMUH.Designation,'')
		...
		LEFT JOIN #TEMP_DEPARTMENT AS TDEPT ON TDEPT.EMPLOYEE_DEPARTMENT = ISNULL(RMUH.Department,'')
