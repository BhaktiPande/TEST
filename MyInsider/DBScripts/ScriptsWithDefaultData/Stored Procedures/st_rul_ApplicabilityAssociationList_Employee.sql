IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_rul_ApplicabilityAssociationList_Employee')
DROP PROCEDURE [dbo].[st_rul_ApplicabilityAssociationList_Employee]
GO
SET ANSI_NULLS ON
GO
...
/*-------------------------------------------------------------------------------------------------  
Description: Procedure to list Applicability association list for applicability allocation - Employee (grid type 114026).  
...
Ashashree  09-Apr-2015   Adding new parameters @inp_nCallingModuleCodeId, @inp_sEmployeeId, @inp_sEmployeeName for module based procedure call identification  
...
Usage:  
EXEC st_rul_ApplicabilityAssociationList_Employee 10, 1, 'EmployeeId','ASC', 132001, 2,  
-------------------------------------------------------------------------------------------------*/  
  
CREATE PROCEDURE [dbo].[st_rul_ApplicabilityAssociationList_Employee]  
 @inp_iPageSize       INT = 10  
 ,@inp_iPageNo       INT = 1  
 ,@inp_sSortField      VARCHAR(255)  
 ,@inp_sSortOrder      VARCHAR(5)  
 ,@inp_nGridTypeCodeId     INT  -- 114026 for Employee Insider , 114091 for Employee non insider.  
...
BEGIN  
 ...
 BEGIN TRY  
  ...
  BEGIN   
   ...
   SELECT @sSQL = @sSQL + ' INNER JOIN rul_ApplicabilityDetails AD ON UI.UserTypeCodeId = AD.InsiderTypeCodeId '  
   SELECT @sSQL = @sSQL + ' INNER JOIN rul_ApplicabilityMaster AM ON AM.ApplicabilityId = AD.ApplicabilityMstId '  
  ...
  (ISNULL(UI.FirstName, '') + ' ' + ISNULL(UI.LastName, '')) AS rul_grd_15046 /*EmployeeName*/,  
