...
BEGIN
	...
	BEGIN TRY
		...
		SELECT TransactionDetailsId, TransactionTypeCodeId,ModeOfAcquisitionCodeId, Quantity,Quantity2,DateOfAcquisition,TM.UserInfoId,ForUserInfoId,TD.SecurityTypeCodeId 
		...
		BEGIN				
				...
				WHERE UserInfoIdRelative=@iForUserInfoId AND SecurityTypeCodeId=@nSecurityTypeCodeId 
								
				SELECT @out_dClosingBalance = ISNULL(TS.ClosingBalance, 0)
				FROM 
				tra_TransactionSummary TS
				WHERE 
				TS.PeriodCodeId=@nPeriodCodeId AND TS.UserInfoIdRelative=@iForUserInfoId  AND TS.SecurityTypeCodeId=@nSecurityTypeCodeId AND TS.YearCodeId=@nYearCodeId
				
					
			IF(@nSecurityTypeCodeId=139001)
			BEGIN
					-- Calculate Pre Percentage			
				--IF(@nTransactionTypeCodeId = @nBuyTransactionType OR @nTransactionTypeCodeId = @nSellTransactionType)OR @nTransactionTypeCodeId = @nESOPTransactionType
