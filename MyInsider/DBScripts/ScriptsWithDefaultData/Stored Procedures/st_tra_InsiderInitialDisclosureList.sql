IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_InsiderInitialDisclosureList')
DROP PROCEDURE [dbo].[st_tra_InsiderInitialDisclosureList]
GO
SET ANSI_NULLS ON
GO
...
GO
/*-------------------------------------------------------------------------------------------------
Description:	Procedure to get Initial Disclosure List for Co Own Securities
...
Usage:
EXEC st_tra_InsiderInitialDisclosureList 
-------------------------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[st_tra_InsiderInitialDisclosureList]
...
OR e.EventCodeId = 153043 -- Confirm Personal Details
OR e.EventCodeId = 153028 )-- Confirm Personal Details
and MapToId IN(
...
WHERE TM.UserInfoId=@inp_sUserInfoId AND TD.DateOfAcquisition>=@dtDateofBecomingInsider AND TM.DisclosureTypeCodeId=147001
UNION
SELECT TM.TransactionMasterId FROM tra_TransactionMaster TM JOIN tra_TransactionDetails TD
ON TM.TransactionMasterId=TD.TransactionMasterId
...
AND TD.DateOfAcquisition>=@dtDateofBecomingInsider AND TM.DisclosureTypeCodeId=147001
)
...
and UserInfoId IN(SELECT UserInfoId FROM usr_UserInfo WHERE usr_UserInfo.UserInfoId=@inp_sUserInfoId AND DateOfBecomingInsider IS NOT NULL)
...
UPDATE t SET t.SoftCopyReq = tm.SoftCopyReq FROM @TempEventLog t JOIN tra_TransactionMaster tm ON tm.TransactionMasterId = t.TMID where  tm.DisclosureTypeCodeId = 147001    
            
UPDATE t SET t.HardCopyReq = tm.HardCopyReq FROM @TempEventLog t JOIN tra_TransactionMaster tm ON tm.TransactionMasterId = t.TMID where  tm.DisclosureTypeCodeId = 147001
...
select TransactionMasterId from tra_TransactionMaster where UserInfoId=@inp_sUserInfoId and DisclosureTypeCodeId=147001 AND TransactionStatusCodeId IN(148003,148004,148007)
...
CREATE TABLE #tUserData (Seq INT identity(1,1),UserInfoId INT,TMID INT, Name VARCHAR(max),Designation VARCHAR(max),
		...
		SELECT 		u.UserInfoId														AS USerInfoId,
					CASE	WHEN t1.TMID IS NOT NULL THEN t1.TMID
							...
							WHEN t8.TMID IS NOT NULL THEN t8.TMID
					...
					CASE WHEN (@UserTypeCodeIdforAdmin=@inp_sUserTypeCodeId OR @UserTypeCodeIdforCO=@inp_sUserTypeCodeId) THEN NULL
					ELSE
					CASE 
		...
		--AND U.DateOfBecomingInsider IS NOT NULL	
		...
		IF (@inp_sSortField  = 'dis_grd_50759' OR @inp_sSortField  = 'dis_grd_50760' OR @inp_sSortField  = 'dis_grd_50761' OR @inp_sSortField  = 'dis_grd_50762' OR @inp_sSortField  = 'dis_grd_50763')
		BEGIN 
		...
		SELECT 
			...
			CASE WHEN TU.UserTypeCodeId=101003 THEN 'Self & Relative' ELSE 'Relative' END AS dis_grd_50759,			 
			DocumentUploadStatus as dis_grd_50760,
			DocumentUploadStatus as DocumentUploadStatus,
			tu.InitialDisclosureDate	 As dis_grd_50761,			
			...
			tu.SoftCopyDate				 As dis_grd_50762,
			tu.TextHardCopyDate			 As TextHardCopyDate,
			tu.HardCopyDate				 As dis_grd_50763,
