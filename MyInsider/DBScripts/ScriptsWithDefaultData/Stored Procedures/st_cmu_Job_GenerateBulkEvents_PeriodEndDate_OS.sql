IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_cmu_Job_GenerateBulkEvents_PeriodEndDate_OS')
DROP PROCEDURE [dbo].[st_cmu_Job_GenerateBulkEvents_PeriodEndDate_OS]
GO
SET ANSI_NULLS ON
GO
...
GO
/*-------------------------------------------------------------------------------------------------
This procedure will populate the eventlog table with events for 'Period End Date for OS'(153069) for furute date only if the future date belongs to the same year-month on which this procedures exectues as a job.
...
Usage:
DECLARE @RC int
EXEC st_cmu_Job_GenerateBulkEvents_PeriodEndDate_OS
-------------------------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[st_cmu_Job_GenerateBulkEvents_PeriodEndDate_OS] 
AS
BEGIN
	...
	BEGIN TRY
		...
		*/
		...
		INNER JOIN 
		(SELECT UserInfoId, MAX(CreatedOn) as CreatedOn FROM tra_UserPeriodEndMapping_OS GROUP BY UserInfoId) t 
	...
	BEGIN CATCH	
		print 'error when saving the event log entries for Period End Date for other security(153069)'
	END CATCH
