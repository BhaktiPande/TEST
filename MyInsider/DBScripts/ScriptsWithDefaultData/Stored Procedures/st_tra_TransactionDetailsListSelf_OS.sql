IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_TransactionDetailsListSelf_OS')
DROP PROCEDURE [dbo].[st_tra_TransactionDetailsListSelf_OS]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[st_tra_TransactionDetailsListSelf_OS]
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @sSQL = @sSQL + ' AND UF.UserTypeCodeId=101003 AND TD.TransactionMasterId = ' + CAST(@inp_iTransactionMasterId AS VARCHAR(15))+' '
		SELECT @sSQL = @sSQL + ' AND UF.UserInfoId = ' + CAST(@inp_nUserInfoId AS VARCHAR(15))+' '
		...
		SELECT	TD.TransactionDetailsId,
				...
				TD.LotSize AS tra_grd_53071,
				TD.ContractSpecification AS tra_grd_53072,
