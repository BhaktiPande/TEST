IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_InitialDisclosureStatusListFor_Employee')
DROP PROCEDURE [st_tra_InitialDisclosureStatusListFor_Employee]
GO
...
CREATE PROCEDURE [dbo].[st_tra_InitialDisclosureStatusListFor_Employee] --523
	 @inp_iUserInfoID				INT
	 ,@inp_iUserTypeID				INT
...
BEGIN
	...
	DECLARE @nEventCode_InitialDiscConfirm INT = 153035
	...
	BEGIN TRY
		...
		IF EXISTS (SELECT * FROM eve_EventLog e WHERE EventCodeId IN (153007) AND UserInfoId = @iParentID AND (e.EventDate<=@dtDateofBecomingInsider OR @dtDateofBecomingInsider IS NULL))
		BEGIN
			SET @nAllDocumentsCompleteFlag = @nStatusFlag_Complete
		END
		...
		BEGIN
			IF EXISTS (SELECT * FROM eve_EventLog e WHERE EventCodeId IN (153007) AND UserInfoId = @inp_iUserInfoID AND (e.EventDate<=@dtDateofBecomingInsider OR @dtDateofBecomingInsider IS NULL))
			BEGIN
			print(1234)			
				...
				SELECT @nTransactionMasterID = MapToId, @ntmpEventCode = EventCodeId, @dttmpEventDate = EventDate 
					FROM eve_EventLog WHERE EventCodeId = 153007 and UserInfoId = @inp_iUserInfoID AND (EventDate<=@dtDateofBecomingInsider OR @dtDateofBecomingInsider IS NULL)
			END
			ELSE
			BEGIN	
			print(123456)			
				...
				IF EXISTS (SELECT * FROM eve_EventLog e WHERE EventCodeId IN (153008) AND UserInfoId = @inp_iUserInfoID AND (e.EventDate<=@dtDateofBecomingInsider OR @dtDateofBecomingInsider IS NULL))
				BEGIN
		...
		WHERE StatusFlag = 154001 AND EventType = 1 AND EL.EventCodeId = 153008
		...
		BEGIN			
			...
			BEGIN
				...
				IF EXISTS (SELECT * FROM eve_EventLog WHERE UserInfoId = @inp_iUserInfoID AND EventCodeId = 153009 AND (EventDate<=@dtDateofBecomingInsider OR @dtDateofBecomingInsider IS NULL))
				BEGIN
					SET @tmpStatusFlag = @nStatusFlag_Complete
					SET @tmpResourceKey = @sHardCopySoftCopyStatus_Done
					SET @SCSubmitted = 1
					SELECT @ntmpEventCode = EventCodeId, @dttmpEventDate = EventDate
						FROM eve_EventLog WHERE UserInfoId = @inp_iUserInfoID AND EventCodeId = 153009
				END
			END
			ELSE
			BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					IF EXISTS (SELECT * FROM eve_EventLog WHERE UserInfoId = @inp_iUserInfoID AND EventCodeId = 153010)
					BEGIN
						SET @tmpStatusFlag = @nStatusFlag_Complete
						SET @tmpResourceKey = @sHardCopySoftCopyStatus_Done
						SET @HCSubmitted = 1
