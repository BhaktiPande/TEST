IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_PolicyDocumentApplicabilityList_Employee')
DROP PROCEDURE [dbo].[st_tra_PolicyDocumentApplicabilityList_Employee]
GO
SET ANSI_NULLS ON
GO
...
GO
/*-------------------------------------------------------------------------------------------------
Description:	Procedure to list users having applicability associated for Policy Document - Employee (grid type = 114036).
...
Usage:
EXEC st_tra_PolicyDocumentApplicabilityList_Employee 10, 1, '','ASC', 2,
-------------------------------------------------------------------------------------------------*/

CREATE PROCEDURE [dbo].[st_tra_PolicyDocumentApplicabilityList_Employee]
...
BEGIN
	...
	BEGIN TRY
		...
		IF @inp_sSortField = 'tra_grd_16051' -- Employee Name
		BEGIN 
			SELECT @inp_sSortField = '(ISNULL(UI.FirstName, '''') + '' '' + ISNULL(UI.LastName, '''')) ' 
		END
		
		IF @inp_sSortField = 'tra_grd_16050' -- EmployeeId
		BEGIN 
			SELECT @inp_sSortField = 'UI.EmployeeId ' 
		END

		IF @inp_sSortField = 'tra_grd_16052' -- Designation
		BEGIN 
		...
		IF @inp_sSortField = 'tra_grd_16053' 
		BEGIN 
			SELECT @inp_sSortField = 'tmpPUE.DocumentViewedDate ' 
		END
		
			IF @inp_sSortField = 'tra_grd_16054'-- EmployeeId
		BEGIN 
		...
		UI.EmployeeId AS tra_grd_16050 /*EmployeeId*/,
		(UI.FirstName + ' ' + UI.LastName) AS tra_grd_16051 /*EmployeeName*/,
		CASE WHEN CdUIDesignation.DisplayCode IS NULL OR CdUIDesignation.DisplayCode = '' THEN CdUIDesignation.CodeName ELSE CdUIDesignation.DisplayCode END AS tra_grd_16052, /*Designation*/
