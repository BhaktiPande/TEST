IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[st_cmu_Job_NotificationQueueGenerator_OS]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[st_cmu_Job_NotificationQueueGenerator_OS]
GO
/****** Object:  StoredProcedure [dbo].[st_cmu_Job_NotificationQueueGenerator_OS]    Script Date: 23-02-2018 10:41:59 ******/
SET ANSI_NULLS ON
GO
...
Usage:
DECLARE @RC int
EXEC st_cmu_Job_NotificationQueueGenerator_OS
-------------------------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[st_cmu_Job_NotificationQueueGenerator_OS] 
AS
BEGIN
	...
	DECLARE @sPlaceholderPreClearanceSecurityToBeTradeQty_UpdatedByCO VARCHAR(100) = '|~|PRE_CLEAR_SECURITY_TO_BE_TRADE_QTY_UPDATEDBY_CO|~|'
	DECLARE @sPlaceholderPreClearanceExpiryDate_UpdatedByCO VARCHAR(50) = '|~|PRE_CLEAR_EXPIRY_DATE_UPDATEDBY_CO|~|'
	DECLARE @sPlaceholderPreClearanceComments_UpdatedByCO VARCHAR(50) = '|~|PRE_CLEAR_COMMENTS_UPDATEDBY_CO|~|'
	...
	DECLARE @nContinuousDisclosureConfirmedCodeId INT = 153061
	...
	--OS
	DECLARE @nPreClearanceRequestedEventCodeID INT = 153045
	DECLARE @nPreClearanceApprovedEventCodeID INT = 153046
	DECLARE @nPreClearanceRejectedEventCodeID INT = 153047
	DECLARE @nPreClearanceExpiryEventCodeID INT = 153067
	DECLARE @nMapToTypeCodeIdPreClearance INT = 132015
	--End
	...
	DECLARE @nPeriodEndDateEventCodeID INT = 153069
	DECLARE @nPeriodEndDisclosureEnteredEventCodeID INT = 153062
	DECLARE @nPeriodEndDisclosureEnteredEventCodeIDSubmittedHardcopy INT=153065
	...
	--OS
	DECLARE @nInitialDisclosureDetailsEnterEventCodeID INT = 153052
	--DECLARE @nInitialDisclosureDocumentUploadEventCodeID INT = 153053
	DECLARE @nInitialDisclosureSoftCopySubmitEventCodeID INT = 153054
	DECLARE @nInitialDisclosureHardCopySubmitEventCodeID INT = 153055
	DECLARE @nInitialDisclosureConfirmEventCodeID INT = 153056
	--end
	
	DECLARE @nDateOfSeperationEventCodeID INT = 153048
	DECLARE @nLastWorkingDayEventCodeID INT = 153049
	...
	BEGIN TRY
		...
		CREATE TABLE #tmpUserWithoutOffset(ID INT IDENTITY(1,1),
											...
											PreClearSecurityToBeTradeQty_UpdatedByCO DECIMAL(15,2),
											PreClearExpiryDate_UpdatedByCO DATETIME NULL,
											PreClearComments_UpdatedByCO VARCHAR(500)
											)
		...
		CREATE TABLE #tmpCOUserToNotify( ID INT IDENTITY(1,1),
										 ...
										 PreClearID VARCHAR(500) NULL, 
										 PreClearStatus NVARCHAR(100) NULL,
										 PreClearDate DATETIME NULL,
										 PreClearExpiryDate DATETIME NULL,
										 PreClearRequestFor VARCHAR(50) NULL, 
										 PreClearDEMATAccNo VARCHAR(50) NULL, 
										 PreClearTransactionType VARCHAR(50) NULL, 
										 PreClearSecurityType VARCHAR(50) NULL, 
										 PreClearSecurityToBeTradeQty DECIMAL(15,2) NULL, 
										 PreClearValueOfProposeTrade DECIMAL(20,2) NULL,
										 PreClearSecurityToBeTradeQty_UpdatedByCO DECIMAL(15,2) NULL,
										 PreClearExpiryDate_UpdatedByCO DATETIME NULL,
										 PreClearComments_UpdatedByCO VARCHAR(500) NULL,
										 NotifyUserFlag	BIT DEFAULT 1
										)
					...
					BEGIN
						...
						BEGIN
							...
							BEGIN
								...
								--Check for trigger and offset events to be Period End Date(153033) and Period End Disclosure details entered(153029,153065) respectively
								IF((CHARINDEX(CAST(@nPeriodEndDateEventCodeID AS VARCHAR),@sRuleTriggerEventCodeIds) > 0) AND 
									((CHARINDEX(CAST(@nPeriodEndDisclosureEnteredEventCodeID AS VARCHAR),@sRuleOffsetEventCodeIds) > 0) OR
									(CHARINDEX(CAST(@nPeriodEndDisclosureEnteredEventCodeIDSubmittedHardcopy AS VARCHAR),@sRuleOffsetEventCodeIds) > 0)))
								BEGIN
									...
									'INSERT INTO #tmpUserWithoutOffset (RuleId, ModeCodeId, UserInfoId , TriggerEventLogId, TriggerEventCodeId, TriggerEventDate, PeriodEndDaysRemain, LastSubmitDate) 
									 ...
									 (	
											...
											,  (select DATEDIFF(DAY, CONVERT(date,EVLTriggerEvents.EventDate),  ( dbo.uf_tra_GetNextTradingDateOrNoOfDaysforPeriodEnd(CONVERT(date, EVLTriggerEvents.EventDate), TP.DiscloPeriodEndToCOByInsdrLimit, null, 0, 1, 0, 116001))))  AS PeriodEndDaysRemain
											...
											INNER JOIN rul_TradingPolicy_OS TP ON EVLTriggerEvents.MapToId = TP.TradingPolicyId
									 ) UsersWithTriggerEvent
									 LEFT JOIN tra_TransactionMaster_OS TM 
								...
								BEGIN
									...
									AND MapToId NOT IN(SELECT DISTINCT MapToId 
									FROM eve_EventLog EL INNER JOIN vw_ApplicableCommunicationRuleForUser RuleUsers 
									ON RuleUsers.RuleId = @nRuleId AND EL.UserInfoId = RuleUsers.UserInfoId AND UserTypeCodeId <> @nUserTypeCodeIdCOUser
									WHERE EventCodeId in (SELECT items FROM dbo.uf_com_Split('' + @sRuleOffsetEventCodeIds + '')))
									SELECT @sSQLInsertToTempTable = ''
									SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable +
									'INSERT INTO #tmpUserWithoutOffset (RuleId, ModeCodeId, UserInfoId , TriggerEventLogId, TriggerEventCodeId, TriggerEventDate, PreClearDate, PreClearStatus, LastSubmitDate, 
																		PreClearID, PreClearRequestFor, PreClearDEMATAccNo, PreClearTransactionType, PreClearSecurityType, PreClearSecurityToBeTradeQty, PreClearValueOfProposeTrade, PreClearExpiryDate_UpdatedByCO, PreClearComments_UpdatedByCO ) 
									 SELECT UsersWithTriggerEvent.RuleId, UsersWithTriggerEvent.CRMMModeCodeId, TriggerEventUserId, TriggerEventLogId, TriggerEventCodeId, TriggerEventDate, PreClearDate, PreClearStatus,  DiscLastSubmitDate, 
											PreClearID, PreClearRequestFor, PreClearDEMATAccNo, PreClearTransactionType, PreClearSecurityType, PreClearSecurityToBeTradeQty, PreClearValueOfProposeTrade, PreClearExpiryDate_UpdatedByCO, PreClearComments_UpdatedByCO  
										FROM
										(
											--Select users for whom the trigger events specified in Rule have occurred
											...
											BEGIN
												...
												--SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' ,PCLR.PreclearanceValidityDateOld AS PreClearExpiryDate '											
												
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , ''PCL'' + CAST(PCLR_TM.DisplayRollingNumber AS VARCHAR(50)) AS PreClearID '
												...
												--SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , PCLR.SecuritiesToBeTradedQty AS PreClearSecurityToBeTradeQty_UpdatedByCO '
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , EL_PCLRExpiry.EventDate AS PreClearExpiryDate_UpdatedByCO '
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , PCLR.ReasonForApproval AS PreClearComments_UpdatedByCO '
											END
											ELSE
											BEGIN
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearDate '	
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearStatus '
												--SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearExpiryDate '
												
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearID '
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearRequestFor '
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearDEMATAccNo '
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearTransactionType '
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearSecurityType '
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearSecurityToBeTradeQty '
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearValueOfProposeTrade '

												--SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearSecurityToBeTradeQty_UpdatedByCO '
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearExpiryDate_UpdatedByCO '
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' , NULL AS PreClearComments_UpdatedByCO '
											END
											...
											IF( (
													...
													(CHARINDEX(CAST(@nInitialDisclosureSoftCopySubmitEventCodeID AS VARCHAR),@sRuleTriggerEventCodeIds) > 0) OR
													(CHARINDEX(CAST(@nInitialDisclosureConfirmEventCodeID AS VARCHAR),@sRuleTriggerEventCodeIds) > 0) 
												) AND 
												(
													(CHARINDEX(CAST(@nInitialDisclosureDetailsEnterEventCodeID AS VARCHAR),@sRuleOffsetEventCodeIds) > 0) OR 
											...
											SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + 
											' FROM cmu_CommunicationRuleMaster CRM '
											-- if communication rule being processed for Employeee Separation 
											IF((CHARINDEX(CAST(@nDateOfSeperationEventCodeID AS VARCHAR),@sRuleTriggerEventCodeIds) > 0) AND 
												(CHARINDEX(CAST(@nLastWorkingDayEventCodeID AS VARCHAR),@sRuleOffsetEventCodeIds) > 0))
											BEGIN
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + 
											' INNER JOIN cmu_CommunicationRuleModeMaster CRMMGlobal ON CRM.RuleId = CRMMGlobal.RuleId AND CRMMGlobal.UserId IS NULL --Get global modes for rule
											...
											SELECT @sMaxTriggerEventTableQuery = @sMaxTriggerEventTableQuery  + 
												...
												(
													...
													INNER JOIN usr_UserInfo UI ON UI.UserInfoId = EVLTriggerEvents1.UserInfoId AND (CONVERT(date,dbo.uf_com_GetServerDate()) <= CONVERT(date,UI.DateOfInactivation))
											...
											SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + ' ON (EVLTriggerEvents.EventCodeId IN (SELECT items FROM dbo.uf_com_Split( ''' + @sRuleTriggerEventCodeIds + ''' )) ) ' 
										...
										ELSE
											...
											BEGIN
												...
												' INNER JOIN tra_PreclearanceRequest_NonImplementationCompany PCLR 
												  ON EVLTriggerEvents.UserInfoId = PCLR.UserInfoId AND EVLTriggerEvents.MapToId = PCLR.PreclearanceRequestId AND EVLTriggerEvents.MapToTypeCodeId = ' + CAST(@nMapToTypeCodeIdPreClearance AS VARCHAR(20)) +
												...
												' INNER JOIN tra_TransactionMaster_OS PCLR_TM ON PCLR.PreclearanceRequestId = PCLR_TM.PreclearanceRequestId ' + -- pre-clearance id
												' INNER JOIN usr_DMATDetails PCLR_Dmat ON PCLR.DMATDetailsID = PCLR_Dmat.DMATDetailsID ' + -- pre-clearance dmat account
												' LEFT JOIN eve_EventLog EL_PCLRExpiry ON EL_PCLRExpiry.UserInfoId = EVLTriggerEvents.UserInfoId AND EL_PCLRExpiry.EventCodeId = ' + CAST(@nPreClearanceExpiryEventCodeID AS VARCHAR(20)) + ' AND EL_PCLRExpiry.MapToTypeCodeId = ' + CAST(@nMapToTypeCodeIdPreClearance AS VARCHAR(20)) + ' AND EL_PCLRExpiry.MapToId = EVLTriggerEvents.MapToId '
											END
											...
											BEGIN
												...
												SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + 
												' LEFT JOIN tra_TransactionMaster_OS TM ON EVLTriggerEvents.UserInfoId = TM.UserInfoId and TM.DisclosureTypeCodeId = 147001
												  LEFT JOIN rul_TradingPolicy_OS TM_TP ON TM.TransactionMasterId IS NOT NULL AND TM.TradingPolicyId = TM_TP.TradingPolicyId
												  LEFT JOIN vw_ApplicableTradingPolicyForUser_OS ATP ON EVLTriggerEvents.UserInfoId = ATP.UserInfoId AND TM.TradingPolicyId = ATP.MapToId
												  LEFT JOIN rul_TradingPolicy_OS ATP_TP ON ATP.MapToId IS NOT NULL AND ATP.MapToId = ATP_TP.TradingPolicyId
							...
							BEGIN 
								...
								BEGIN
									...
									SELECT @sSQLInsertToTempTable = ''
									SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable +
									'INSERT INTO #tmpUserWithoutOffset (RuleId, ModeCodeId, UserInfoId , TriggerEventLogId, TriggerEventCodeId, TriggerEventDate, PreClearDate, PreClearStatus
																			...
																			, PeriodEndDaysRemain, LastSubmitDate, PreClearExpiryDate_UpdatedByCO, PreClearComments_UpdatedByCO )
									SELECT UsersWithTriggerEvent.RuleId, UsersWithTriggerEvent.CRMMModeCodeId, TriggerEventUserId, TriggerEventLogId, TriggerEventCodeId, TriggerEventDate, PreClearDate, PreClearStatus
											, PreClearID, PreClearRequestFor, PreClearDEMATAccNo, PreClearTransactionType, PreClearSecurityType, PreClearSecurityToBeTradeQty, PreClearValueOfProposeTrade,
											 PeriodEndDaysRemain, LastSubmitDate, PreClearExpiryDate_UpdatedByCO, PreClearComments_UpdatedByCO    
									FROM
									(
							...
							BEGIN
								...
								(
									...
									(
										...
										--BEGIN
										--	SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + 
										--	' INNER JOIN tra_TransactionMaster_OS TRM ON EVE.UserInfoId = TRM.UserInfoId AND EVE.MapToTypeCodeId = ' + CAST(@nMapToTypeCodeIdDisclosureTransaction AS VARCHAR(20)) + ' AND EVE.MapToId = TRM.TransactionMasterId '
										--	SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable + 
										--	' INNER JOIN rul_TradingPolicy_OS TP ON TRM.TradingPolicyId = TP.TradingPolicyId AND TP.StExSubmitDiscloToStExByCOHardcopyFlag = 1 '
										--END
									SELECT @sSQLInsertToTempTable = @sSQLInsertToTempTable 
		...
		REPLACE(  REPLACE( REPLACE( REPLACE(REPLACE( REPLACE( TM.Subject,'&lt;','<') 
															  ,'&gt;','>')
															  ,'&quot;','"')
															  ,'&#39;','''')
															  ,'&amp;','&')
		...
		, @sPlaceholderPreClearanceSecurityToBeTradeQty_UpdatedByCO, ISNULL(CAST(dbo.uf_com_FormatNumberToCurrency(UsrWOOffset.PreClearSecurityToBeTradeQty_UpdatedByCO ,'IND') as VARCHAR(50)) ,''))
		, @sPlaceholderPreClearanceExpiryDate_UpdatedByCO, (ISNULL(REPLACE(CONVERT(VARCHAR(20), UsrWOOffset.PreClearExpiryDate_UpdatedByCO, 106),' ','/'),'')))
		, @sPlaceholderPreClearanceComments_UpdatedByCO, ISNULL(UsrWOOffset.PreClearComments_UpdatedByCO ,''))
		,'&gt;','>')
		,'&quot;','"')
	    ,'&#39;','''')
		,'&amp;','&')
		...
		,'&#233;','é')
		AS Contents,
		 REPLACE( REPLACE( REPLACE( REPLACE( REPLACE(REPLACE(REPLACE(TM.Signature,'\r\n','<br />') 
															  ,'&gt;','>')
															  ,'&quot;','"')
															  ,'&#39;','''')
															  ,'&amp;','&')
		...
		LEFT JOIN mst_Company CMP ON UI.CompanyId = CMP.CompanyId
		...
		CASE WHEN CRMMPersonal.ModeCodeId IS NULL 
			...
			ELSE /*CRMMPersonal.ModeCodeId IS NOT NULL*/
				...
				END 
		END AS UserContactInfo,
		REPLACE( REPLACE( REPLACE( REPLACE(REPLACE( REPLACE( REPLACE(TM.Subject, @sPlaceholderPolicyDocumentName, ' ''' + ISNULL(PD.PolicyDocumentName,'') + ''' ')
															  ,'&lt;','<') 
															  ,'&gt;','>')
															  ,'&quot;','"')
															  ,'&#39;','''')
		...
		--select * from cmu_NotificationQueue
		...
		CASE WHEN CRMMPersonal.ModeCodeId IS NULL 
			...
			ELSE /*CRMMPersonal.ModeCodeId IS NOT NULL*/
				...
				END 
		END AS UserContactInfo,
	 	REPLACE( REPLACE( REPLACE( REPLACE(REPLACE( REPLACE( 	REPLACE(REPLACE(TM.Subject, @sPlaceholderPolicyDocumentName, ' '''+ ISNULL(PD.PolicyDocumentName,'') + ''' ') 
		...
		LEFT JOIN rul_TradingPolicy_OS TP ON TP.TradingPolicyId = COUsrWOOffset.TriggerMapToId AND COUsrWOOffset.TriggerMapToTypeCodeId = @nMapToTypeCodeIdTradingPolicy AND COUsrWOOffset.TriggerEventCodeId = @nTradingPolicyExpiryEventCodeID
		LEFT JOIN mst_Company CMP ON UI.CompanyId = CMP.CompanyId
		LEFT JOIN com_Code CCGrade ON UI.GradeId = CCGrade.CodeID
		LEFT JOIN com_Code CCCountry ON UI.CountryId = CCCountry.CodeID
		LEFT JOIN com_Code CCCategory ON UI.Category = CCCategory.CodeID
			...
			BEGIN TRY
				...
				BEGIN
					...
					---------------------End - Generate SQL Error----------------------
						...
						--Update #tmpCOUserToNotify when pre-clearance requested. 
						UPDATE #tmpCOUserToNotify SET UserFirstName = UI.FirstName, UserLastName = UI.LastName, PreClearID = 'PCL' + CAST(PCLR_TM.DisplayRollingNumber AS VARCHAR(50)), PreClearStatus = CCPreClearStatus.CodeName, 
							   PreClearDate = PCLR.CreatedOn, PreClearRequestFor = CCPreClearRquestFor.CodeName, 
							   PreClearDEMATAccNo = PCLR_Dmat.DEMATAccountNumber, PreClearTransactionType = CCPreClearTranType.CodeName, PreClearSecurityType = CCPreClearSecuType.CodeName, 
							   PreClearSecurityToBeTradeQty = PCLR.SecuritiesToBeTradedQty, PreClearValueOfProposeTrade = PCLR.SecuritiesToBeTradedValue,
							   PreClearComments_UpdatedByCO = PCLR.ReasonForApproval
						FROM #tmpCOUserToNotify tCONotify 
						INNER JOIN #tmpCOUserToNotifyEventBased TCOToNEB ON tCONotify.RuleId = TCOToNEB.RuleId AND tCONotify.COUserInfoId = TCOToNEB.COUserInfoId
						INNER JOIN eve_EventLog evelog 
						ON evelog.UserInfoId = TCOToNEB.UserInfoId AND evelog.MapToId = TCOToNEB.TriggerMapToId AND evelog.EventLogId = TCOToNEB.TriggerEventLogId AND evelog.MapToTypeCodeId = TCOToNEB.TriggerMapToTypeCodeId
						INNER JOIN usr_UserInfo UI ON TCOToNEB.UserInfoId = UI.UserInfoId
						INNER JOIN tra_PreclearanceRequest_NonImplementationCompany PCLR ON TCOToNEB.UserInfoId = PCLR.UserInfoId AND TCOToNEB.TriggerMapToId = PCLR.PreclearanceRequestId 
						INNER JOIN com_Code CCPreClearStatus ON PCLR.PreclearanceStatusCodeId = CCPreClearStatus.CodeId
						INNER JOIN com_Code CCPreClearRquestFor ON PCLR.PreclearanceRequestForCodeId = CCPreClearRquestFor.CodeId  
						INNER JOIN com_Code CCPreClearTranType ON PCLR.TransactionTypeCodeId = CCPreClearTranType.CodeId  
						INNER JOIN com_Code CCPreClearSecuType ON PCLR.SecurityTypeCodeId = CCPreClearSecuType.CodeId 
						INNER JOIN tra_TransactionMaster_OS PCLR_TM ON PCLR.PreclearanceRequestId = PCLR_TM.PreclearanceRequestId
						INNER JOIN usr_DMATDetails PCLR_Dmat ON PCLR.DMATDetailsID = PCLR_Dmat.DMATDetailsID 
						WHERE tCONotify.NotifyUserFlag = 1 AND TCOToNEB.NotifyUserFlag = 1
						...
						SELECT @sNotificationContents=Replace(@sNotificationContents,'[[','[')
						--@inp_sContents=Replace(@inp_sContents,'[[','[')
						INSERT INTO cmu_NotificationQueue(CompanyIdentifierCodeId, RuleModeId, ModeCodeId ,EventLogId, 
						UserId, UserContactInfo, [Subject], Contents, [Signature], CommunicationFrom, CreatedBy, CreatedOn, ModifiedBy, ModifiedOn)
						SELECT 
						--UsrWOOffset.NotifyUserFlag, UsrWOOffset.RuleId,
						NULL AS CompanyIdentifierCodeId
						,CASE WHEN CRMMPersonal.RuleModeId IS NULL THEN CRMMGlobal.RuleModeId ELSE CRMMPersonal.RuleModeId END AS RuleModeId
						...
						,REPLACE(REPLACE( REPLACE( REPLACE(REPLACE( REPLACE( TM.Subject  ,'&lt;','<') 
															  ...
															  ,'&amp;','&')
															   ,'&#233;','é')AS Subject,
						REPLACE( REPLACE( REPLACE( REPLACE( REPLACE(REPLACE(
						 REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(@sNotificationContents,'\r\n','<br />')
						, @sPlaceholderLoginId, ISNULL(UA.LoginID,''))
						, @sPlaceholderAppURL, @sAppURLCodeValue +'?code='+ ISNULL(URP.HashCode,''))
						, @sPlaceholderAppURLAnchorTag, '<a target="blank" href="'+@sAppURLCodeValue+'?code='+ISNULL(URP.HashCode,'')+'" >'+@sPlaceholderAppURLAnchorText+'</a>')
						, @sPlaceholderFirstName, CASE WHEN COUserToNotify.UserFirstName IS NULL THEN ISNULL(UI.FirstName, ISNULL(UI.ContactPerson,'')) ELSE COUserToNotify.UserFirstName END)
						, @sPlaceholderMiddleName, ISNULL(UI.MiddleName, ''))
						, @sPlaceholderLastName, CASE WHEN COUserToNotify.UserLastName IS NULL THEN ISNULL(UI.LastName, '') ELSE COUserToNotify.UserLastName END)
						, @sPlaceholderUserID, ISNULL(UI.EmployeeId,'-'))
						, @sPlaceholderCompanyName, ISNULL(CMP.CompanyName, '-'))
						, @sPlaceholderEmailID, ISNULL(UI.EmailId,'-'))
						, @sPlaceholderPreClearanceID, ISNULL(COUserToNotify.PreClearID ,''))
						, @sPlaceholderPreClearanceStatus, ISNULL(COUserToNotify.PreClearStatus ,'') )
						, @sPlaceholderPreClearanceRequestFor, ISNULL(COUserToNotify.PreClearRequestFor,''))
						, @sPlaceholderPreClearanceDEMATAccNo, ISNULL(COUserToNotify.PreClearDEMATAccNo ,''))
						, @sPlaceholderPreClearanceTransactionType, ISNULL(COUserToNotify.PreClearTransactionType,''))
						, @sPlaceholderPreClearanceSecurityType, ISNULL(COUserToNotify.PreClearSecurityType ,''))
						, @sPlaceholderPreClearanceCreateDate, (ISNULL(REPLACE(CONVERT(VARCHAR(20), COUserToNotify.PreClearDate, 106),' ','/'),'')))
						, @sPlaceholderPreClearanceExpiryDate, (ISNULL(REPLACE(CONVERT(VARCHAR(20), COUserToNotify.PreClearExpiryDate, 106),' ','/'),'')))
						, @sPlaceholderPreClearanceSecurityToBeTradeQty, ISNULL(CAST(dbo.uf_com_FormatNumberToCurrency(COUserToNotify.PreClearSecurityToBeTradeQty ,'IND') as VARCHAR(50)) ,''))
						, @sPlaceholderPreClearanceValueOfPropseTrade, ISNULL(CAST(COUserToNotify.PreClearValueOfProposeTrade as VARCHAR(50)) ,''))
						, @sPlaceholderPreClearanceSecurityToBeTradeQty_UpdatedByCO, ISNULL(CAST(dbo.uf_com_FormatNumberToCurrency(COUserToNotify.PreClearSecurityToBeTradeQty_UpdatedByCO ,'IND') as VARCHAR(50)) ,''))
						, @sPlaceholderPreClearanceExpiryDate_UpdatedByCO, (ISNULL(REPLACE(CONVERT(VARCHAR(20), COUserToNotify.PreClearExpiryDate_UpdatedByCO, 106),' ','/'),'')))
						, @sPlaceholderPreClearanceComments_UpdatedByCO, ISNULL(COUserToNotify.PreClearComments_UpdatedByCO ,''))
						, @sPlaceholderMobileNumber, ISNULL(UI.MobileNumber, ISNULL(UI.LandLine1,ISNULL(UI.LandLine2,'-'))) )
						, @sPlaceholderGrade, (CASE WHEN UI.GradeId IS NULL THEN ISNULL(UI.GradeText,'-') ELSE (CASE WHEN CCGrade.DisplayCode IS NULL OR CCGrade.DisplayCode = '' THEN CCGrade.CodeName ELSE CCGrade.DisplayCode END) END)
						)
						,'&gt;','>')
