IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_PeriodEndDisclosureUsersStatus_OS')
DROP PROCEDURE [dbo].[st_tra_PeriodEndDisclosureUsersStatus_OS]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[st_tra_PeriodEndDisclosureUsersStatus_OS]
...
BEGIN
	...
	DECLARE @dtCurrentDate 							DATETIME = CONVERT(date, dbo.uf_com_GetServerDate()) -- '2019-10-01'
	...
	BEGIN TRY
	 ...
	 DetailsSubmitStatus INT, DetailsSubmitDate DATETIME, ScpSubmitStatus INT, ScpSubmitDate DATETIME,HcpSubmitStatus INT, HcpSubmitDate DATETIME)
		...
		From 
			(
				SELECT UserInfoId, MAX(PeriodEndDate) as PeriodEndDate FROM #TempPeriodEnd_EventStatus vwPEDS
				GROUP BY UserInfoId
		...
		FROM #tmpPeriodEndDisclosureUserList Tmp JOIN #TempPeriodEnd_EventStatus vwPEDS ON
		...
		SET
			...
			SubmissionLastDate = CONVERT(date, dbo.uf_tra_GetNextTradingDateOrNoOfDaysforPeriodEnd_OS(TM.PeriodEndDate, ISNULL(TP.DiscloPeriodEndToCOByInsdrLimit, 0), NULL, 0, 1, 0, @nExchangeTypeCodeId_NSE)),-- DATEADD(D, ISNULL(TP.DiscloPeriodEndToCOByInsdrLimit, 0), TM.PeriodEndDate),
		...
		BEGIN 
			...
			FROM #tmpPeriodEndDisclosureUserList tmp 
			JOIN rul_TradingPolicy_OS TP ON tmp.TradingPolicyId = TP.TradingPolicyId AND tmp.Id = @nTempId
		...
		UPDATE Tmp 
		SET 
			SubmissionLastDate = CONVERT(date, dbo.uf_tra_GetNextTradingDateOrNoOfDaysforPeriodEnd_OS(Tmp.PeriodEndDate, ISNULL(TP.DiscloPeriodEndToCOByInsdrLimit, 0), NULL, 0, 1, 0, @nExchangeTypeCodeId_NSE)), -- DATEADD(D, ISNULL(TP.DiscloPeriodEndToCOByInsdrLimit, 0), Tmp.PeriodEndDate),
			SubmissionByCOLastDate = CONVERT(date, dbo.uf_tra_GetNextTradingDateOrNoOfDaysforPeriodEnd_OS(Tmp.PeriodEndDate, ISNULL(TP.DiscloPeriodEndToCOByInsdrLimit, 0) + ISNULL(TP.DiscloPeriodEndSubmitToStExByCOLimit, 0), NULL, 0, 1, 0, @nExchangeTypeCodeId_NSE)) --DATEADD(D, ISNULL(TP.DiscloPeriodEndToCOByInsdrLimit, 0) + ISNULL(TP.DiscloPeriodEndSubmitToStExByCOLimit, 0), Tmp.PeriodEndDate)
		FROM #tmpPeriodEndDisclosureUserList Tmp LEFT JOIN rul_TradingPolicy_OS TP ON Tmp.TradingPolicyId = TP.TradingPolicyId 
		...
		FROM #tmpPeriodEndDisclosureUserList Tmp LEFT JOIN eve_EventLog EL on 
				Tmp.UserInfoId=EL.UserInfoId AND MapToTypeCodeId = 132005 AND EventCodeId = 153056
		...
		SET SubmissionDaysRemaining = CASE WHEN SubmissionLastDate >= @dtCurrentDate THEN CONVERT(int, dbo.uf_tra_GetNextTradingDateOrNoOfDaysforPeriodEnd_OS(@dtCurrentDate, NULL, SubmissionLastDate, 0, 0, 0, @nExchangeTypeCodeId_NSE)) ELSE -1 END, --DATEDIFF(D, @dtCurrentDate, SubmissionLastDate)
			SubmissionDaysRemainingByCO = CASE WHEN SubmissionByCOLastDate >= @dtCurrentDate THEN CONVERT(int, dbo.uf_tra_GetNextTradingDateOrNoOfDaysforPeriodEnd_OS(@dtCurrentDate, NULL, SubmissionByCOLastDate, 0, 0, 0, @nExchangeTypeCodeId_NSE)) ELSE -1 END -- DATEDIFF(D, @dtCurrentDate, SubmissionByCOLastDate)
		...
		on tPE.UserInfoId=EL.UserInfoId
		 where  EventCodeId in(153063) and EventCodeId not in(153064))
		 END	
		...
		IF @inp_sSortField = 'dis_grd_51014' -- Employee ID
		BEGIN 
			SELECT @inp_sSortField = 'EmployeeId ' 
		END
		
		IF @inp_sSortField = 'dis_grd_51015' -- Employee Name
		BEGIN 
			SELECT @inp_sSortField = 'InsiderName ' 
		END
		
		IF @inp_sSortField = 'dis_grd_51018' -- Designation
		BEGIN 
		...
		SELECT
			...
			PeriodCodeId,			
			UserInfoId,
			EmployeeId as dis_grd_51014,
			InsiderName as dis_grd_51015,
			CompanyId, 
			CompanyName, 
			UserType, 
			DesignationId, 
			Designation as dis_grd_51018,
			UserPanNumber as dis_grd_51016,
			...
			END  AS dis_grd_51017,
			SubmissionLastDate as dis_grd_51019,
			
			SubmissionEventDate as dis_grd_51020,
			SubmissionButtonText,
			SubmissionStatusCodeId,
			
			ScpEventDate as dis_grd_51021,
			ScpButtonText,
			ScpStatusCodeId,
			
			HcpEventDate as dis_grd_51022,
			HcpButtonText,
			HcpStatusCodeId,
			
			--HCpByCOEventDate as dis_grd_51023,
			HCpByCOButtonText,
			HCpByCOStatusCodeId,
			...
			CCatagary.CodeName As dis_grd_51025
