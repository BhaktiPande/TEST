...
IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_usr_CreateJob_InactivateUser')
DROP PROCEDURE [dbo].[st_usr_CreateJob_InactivateUser]
GO
SET ANSI_NULLS ON
GO
...
Description:	This procedure when executed creats a job which will run on daily basis. 
				The job created runs continuously after every day and executes procedure "st_cmu_CreateJob_InactivateUser" 
				to Inactivate user where Inactivation Date is less than equal to todays date.
...
Parag			13-Apr-2015		Made change job schedule - this job will run on every day on 00:01:00 am 

-------------------------------------------------------------------------------------------------*/

CREATE PROCEDURE st_usr_CreateJob_InactivateUser
(
...
BEGIN
	...
	DECLARE @ERR_JOB_CREATE_INACTIVEUSER INT = -1

	BEGIN TRY
		--USE [msdb]
		/*--------- CREATE NotificationQueueGenerator JOB ---------------------*/
		DECLARE @ReturnCode INT
		SET @sJobName = 'InactivateUser' + CAST(@inp_nMasterDBCompanyId AS VARCHAR(10)) + '_' + @sDBName
		...
		EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@sJobName, 
				...
				@notify_level_page=0, 
				@delete_level=0, 
				@description=N'Job to Inactivate user where Inactivation Date is less than equal to todays date.', 
		...
		EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, 
				...
				EXECUTE @RC = [dbo].[st_usr_Job_InactivateUser] 
				GO', 
		...
		SET @sScheduleName = N'InactivateUser'+ CAST(@inp_nMasterDBCompanyId AS VARCHAR(10)) +'_Daily'
		EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, 
				...
				@active_start_time=000100, --Start time is set as midnight timing 00:01:00am
	...
	BEGIN CATCH
		...
		RETURN @ERR_JOB_CREATE_INACTIVEUSER
	END CATCH
END