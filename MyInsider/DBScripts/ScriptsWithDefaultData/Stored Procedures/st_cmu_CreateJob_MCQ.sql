...
IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_cmu_CreateJob_MCQ')
DROP PROCEDURE [dbo].[st_cmu_CreateJob_MCQ]
GO
SET ANSI_NULLS ON
GO
...
Description:	This procedure when executed creats a job which will run on daily basis. 
				The job created runs continuously after every 15 mins and executes procedure "st_cmu_CreateJob_MCQ" 
				to unlock blocked user before next frequency
...
EXEC st_cmu_CreateJob_MCQ  19,'Vigilante_Samadhan'
-------------------------------------------------------------------------------------------------*/

CREATE PROCEDURE st_cmu_CreateJob_MCQ
(
...
BEGIN
	...
	BEGIN TRY
		...
		SET @sJobName = 'KPCSVigilanteCompMCQ' + CAST(@inp_nMasterDBCompanyId AS VARCHAR(10)) + '_' + @sDBName
		...
		EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@sJobName, 
				...
				@notify_level_page=0, 
				@delete_level=0, 
				@description=N'Job to unlock blocked user before next frequency', 
		...
		EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, 
				...
				EXECUTE @RC = [dbo].[st_MCQ_UnblockUserForNextFrequency] 
				GO', 
		...
		SET @sScheduleName = N'KPCSVigilanteCompMCQ'+ CAST(@inp_nMasterDBCompanyId AS VARCHAR(10)) +'_DailyEvery_15min'
