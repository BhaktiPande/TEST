...
IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_rpt_CreateJob_DefaulterGenerator_OS')
DROP PROCEDURE [dbo].[st_rpt_CreateJob_DefaulterGenerator_OS]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE st_rpt_CreateJob_DefaulterGenerator_OS
(
...
BEGIN
	...
	BEGIN TRY
		...
		SET @sJobName = 'DefaulterReport_OS_' + CAST(@inp_nMasterDBCompanyId AS VARCHAR(10)) + '_' + @sDBName
		...
		EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, 
				...
				EXECUTE @RC = [dbo].[st_rpt_Job_DefaulterGenerator_OS] 
				GO', 
		...
		SET @sScheduleName = N'DefaulterReport_OS'+ CAST(@inp_nMasterDBCompanyId AS VARCHAR(10)) +'_Daily'
