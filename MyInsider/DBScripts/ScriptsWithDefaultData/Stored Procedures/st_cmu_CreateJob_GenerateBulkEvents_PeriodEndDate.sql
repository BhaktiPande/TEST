...
IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_cmu_CreateJob_GenerateBulkEvents_PeriodEndDate')
DROP PROCEDURE [dbo].[st_cmu_CreateJob_GenerateBulkEvents_PeriodEndDate]
GO
SET ANSI_NULLS ON
GO
...
Description:	This procedure when executed creats a job which will run on monthly basis. 
				The job created after exectuing this procedure, will run once every month and execute procedure "st_cmu_Job_GenerateBulkEvents_PeriodEndDate" 
				to generate eventlog entries in bulk, into table "eve_EventLog" for events 'Period End Date (153033)'
...
Parag			13-Apr-2015		Made change to create job which run daily instead of once in month 
								also change job schedule - this job will run on every day on 00:30:00 am 

-------------------------------------------------------------------------------------------------*/

CREATE PROCEDURE st_cmu_CreateJob_GenerateBulkEvents_PeriodEndDate
(
...
BEGIN
	...
	BEGIN TRY
		...
		EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, 
				...
				EXECUTE @RC = [dbo].[st_cmu_Job_GenerateBulkEvents_PeriodEndDate] 
				GO', 
