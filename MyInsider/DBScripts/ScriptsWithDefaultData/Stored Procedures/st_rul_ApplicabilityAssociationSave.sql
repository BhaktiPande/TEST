...
DROP PROCEDURE [dbo].[st_rul_ApplicabilityAssociationSave]
GO
SET ANSI_NULLS ON
GO
...
Ashashree		11-Jun-2015		Adding validations for Communication Rule related applicability
Ashashree		13-Jul-2015		Adding call to procedure 'st_rul_UserwiseOverlapTradingPolicyCount' to get the count of users who have overlapping trading policy
Gaurishankar	02-Feb-2016		Added codition for If @inp_nAllEmployeeFlag = 1  then AllEmployeeInsiderFlag should not be 1 for applicablity change for All Employee.
Gaurishankar	25-Apr-2016		Changes for AllCo, AllCorporateInsider, AllNonEmployeeInsider and Non Insider Employee Applicability save.
Raghvendra		07-Sep-2016		Changed the GETDATE() call with function dbo.uf_com_GetServerDate(). This function will return the date to be used as server date.

Usage:
DECLARE @RC int
EXEC st_rul_ApplicabilityAssociationSave 132006, 4, 0, 0, 0, @inp_tblApplicabilityFilterType, @inp_tblApplicabilityIncludeExcludeUsers
-------------------------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[st_rul_ApplicabilityAssociationSave] 
...
BEGIN
	...
	BEGIN TRY
		...
		ELSE IF( (@inp_nMapToTypeCodeId = @nMapToTypeCodeId_TradingPolicy) AND 
			(NOT EXISTS(SELECT TradingPolicyId FROM rul_TradingPolicy WHERE TradingPolicyId = @inp_nMapToId)) )
		BEGIN	
		...
		BEGIN
			SELECT @nMapType_ExistingStatus = TradingPolicyStatusCodeId FROM rul_TradingPolicy WHERE TradingPolicyId = @inp_nMapToId
		END
		print ' 2-- '
		...
		print '4-- '
		--Insert into rul_ApplicabilityMaster with @inp_nMapToTypeCodeId, @inp_nMapToId and @nApplicabilityVersionNumber 
		...
		*/
		...
		BEGIN
			INSERT INTO rul_ApplicabilityDetails(ApplicabilityMstId, AllEmployeeFlag, AllInsiderFlag, AllEmployeeInsiderFlag,AllCo,AllCorporateEmployees,AllNonEmployee, CreatedBy, CreatedOn, ModifiedBy, ModifiedOn) 
