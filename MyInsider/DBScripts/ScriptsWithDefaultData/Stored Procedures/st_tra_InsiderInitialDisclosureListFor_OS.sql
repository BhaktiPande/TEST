IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_InsiderInitialDisclosureListFor_OS')
DROP PROCEDURE [dbo].[st_tra_InsiderInitialDisclosureListFor_OS]
GO
SET ANSI_NULLS ON
GO
...
GO
/*-------------------------------------------------------------------------------------------------
Description:	Procedure to get Initial Disclosure List for Insider for Other Securities
...
Usage:
EXEC st_tra_InsiderInitialDisclosureListFor_OS 
-------------------------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[st_tra_InsiderInitialDisclosureListFor_OS]
...
CREATE TABLE #tUserData (Seq INT identity(1,1),UserInfoId INT,TMID INT, Name VARCHAR(max),Designation VARCHAR(max),
		...
		FROM		usr_UserInfo u		
					...
					LEFT JOIN @TempEventLog t2 ON t2.UserInfoId = u.UserInfoId and t2.EventType = 153052 -- Initial Disclosure details entered for OS
		...
		IF (@inp_sSortField  = 'dis_grd_52006' OR @inp_sSortField  = 'dis_grd_52007' OR @inp_sSortField  = 'dis_grd_52008' OR @inp_sSortField  = 'dis_grd_52009' OR @inp_sSortField  = 'dis_grd_52010')
		BEGIN 
		...
		SELECT 
			...
			CASE WHEN TU.UserTypeCodeId=101003 THEN 'Self & Relative' ELSE 'Relative' END AS dis_grd_52006,			 
			DocumentUploadStatus as dis_grd_52007,
			DocumentUploadStatus as DocumentUploadStatus,
			tu.InitialDisclosureDate	 As dis_grd_52008,			
			...
			tu.SoftCopyDate				 As dis_grd_52009,
			tu.TextHardCopyDate			 As TextHardCopyDate,
			tu.HardCopyDate				 As dis_grd_52010,					
