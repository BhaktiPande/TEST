...
IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_rpt_PreclearanceEmployeeIndividualForSSRS')
	DROP PROCEDURE st_rpt_PreclearanceEmployeeIndividualForSSRS
GO
SET ANSI_NULLS ON
GO
...
/*-------------------------------------------------------------------------------------------------
Description:	Procedure to List data for employee wise Individual report
...
CREATE PROCEDURE [dbo].[st_rpt_PreclearanceEmployeeIndividualForSSRS]
	...
	,@inp_dtPreclearanceDateFrom	DATETIME
	,@inp_dtPreclearanceDateTo	DATETIME
...
BEGIN
	...
	ScripName VARCHAR(100), ISIN VARCHAR(100), TransactionTypeCodeId INT, SecurityTypeCodeId INT, PreQuantity DECIMAL(25, 4), PreValue DECIMAL(25, 4),
	...
	CREATE TABLE #tmpComments(CodeId INT, DisplayText VARCHAR(100))
	...
	BEGIN TRY
		...
		BEGIN
			...
			SELECT @sSQL = @sSQL + 'AND DisclosureTypeCodeId <> 147001 AND TM.UserInfoId IN (' + CONVERT(VARCHAR(max), @inp_iUserInfoId) + ' ) '
			...
			IF ((@inp_sPreclearanceID IS NOT NULL AND @inp_sPreclearanceID <> '')
			  ...
			  OR (@inp_dtPreclearanceDateFrom IS NOT NULL AND @inp_dtPreclearanceDateFrom <> '')
			  OR (@inp_dtPreclearanceDateTo IS NOT NULL AND @inp_dtPreclearanceDateTo <> ''))
			BEGIN
				IF (@inp_sPreclearanceID IS NOT NULL AND @inp_sPreclearanceID <> '')
				BEGIN
				...
				END
				IF (@inp_dtPreclearanceDateFrom IS NOT NULL AND @inp_dtPreclearanceDateFrom <> '')
				BEGIN
					SELECT @sSQL = @sSQL + 'AND CONVERT(DATE, ''' + CONVERT(VARCHAR(11), @inp_dtPreclearanceDateFrom) + ''') <= CONVERT(DATE,PR.CreatedOn) '
				END
				
				IF (@inp_dtPreclearanceDateTo IS NOT NULL AND @inp_dtPreclearanceDateTo <> '')
				BEGIN
					SELECT @sSQL = @sSQL + 'AND CONVERT(DATE, ''' + CONVERT(VARCHAR(11), @inp_dtPreclearanceDateTo) + ''') >= CONVERT(DATE,PR.CreatedOn )'
				END
			...
			SELECT DISTINCT TM.TransactionMasterId, 
					...
					PR.SecuritiesToBeTradedQty, PR.SecuritiesToBeTradedValue, PR.PreclearanceStatusCodeId, ELPre.EventDate,
			...
			UPDATE tmpData
			SET CommentId_TrdWithoutPcl = @nPreclearanceComment_TrdWithoutPcl
			FROM #tmpPreclearance tmpData
			...
			SELECT 
				tmpData.UserInfoId,
				tmpData.TransactionMasterIdText  AS rpt_grd_19206,
				 dbo.uf_rpt_FormatDateValue(Requestdate,0) AS rpt_grd_19207,
				 Scripname AS rpt_grd_19208,
				 ISIN  AS rpt_grd_19209,
				 CTransaction.CodeName  AS rpt_grd_19210,
				 CSecurity.CodeName  AS rpt_grd_19211,
				 dbo.uf_rpt_FormatValue(CONVERT(VARCHAR(max),PreQuantity),1) AS rpt_grd_19212,
				 dbo.uf_rpt_FormatValue(CONVERT(VARCHAR(max),PreValue),1) AS rpt_grd_19213,
				 dbo.uf_rpt_FormatValue(CONVERT(VARCHAR(max),CPreStatus.CodeName),1) AS rpt_grd_19214,
				 dbo.uf_rpt_FormatDateValue(PreStatusDate,0) AS rpt_grd_19215,
				 dbo.uf_rpt_FormatDateValue(PreApplicableTill,0)  AS rpt_grd_19216,
				...
				CASE WHEN TM.ParentTransactionMasterId IS NULL THEN dbo.uf_rpt_FormatDateValue(DateOfAcquisition,0) ELSE '' END AS rpt_grd_19220,
