	...
	BEGIN TRY
		...
		IF @out_sSQLErrMessage IS NULL
			...
			--IF @inp_iSellAllDetailsId=0
				...
				END
				SELECT @nSellAllDetailsId = SCOPE_IDENTITY()
			
			--ELSE
				
			--IF (EXISTS(SELECT SellAllDetailsId FROM tra_SellAllValues_OS WHERE TransactionMasterId = @inp_iTransactionMasterId and 
			--		SellAllFlag=@inp_bSellAllFlag and ForUserInfoId=@inp_iForUserInfoId and 
			--		CompanyId=@inp_iCompanyId and DMATDetailsId= @inp_iDMATDetailsId))		
			--	BEGIN												
					
			--			UPDATE tra_SellAllValues_OS
			--			SET 	
			--			 TransactionMasterId = @inp_iTransactionMasterId
			--			,SellAllFlag = @inp_bSellAllFlag
			...
			--			WHERE SellAllDetailsId = @nSellAllDetailsId
			--			SELECT @nSellAllDetailsId = SCOPE_IDENTITY()
			--	END	
		...
		Join usr_UserInfo UI ON SA.ForUserInfoId=UI.UserInfoId
		WHERE SellAllDetailsId=@nSellAllDetailsId 
