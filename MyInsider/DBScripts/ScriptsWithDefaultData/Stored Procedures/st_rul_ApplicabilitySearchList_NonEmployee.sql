IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_rul_ApplicabilitySearchList_NonEmployee')
DROP PROCEDURE [dbo].[st_rul_ApplicabilitySearchList_NonEmployee]
GO
SET ANSI_NULLS ON
GO
...
GO
/*-------------------------------------------------------------------------------------------------
Description:	Procedure to list Applicability search list for applicability allocation - Non Employee .
...
Ashashree		12-Apr-2015			Modifying query to fetch the non-employee users for whom applicability is not associated
Raghvendra		07-Sep-2016		Changed the GETDATE() call with function dbo.uf_com_GetServerDate(). This function will return the date to be used as server date.

Usage:
EXEC st_rul_ApplicabilitySearchList_NonEmployee 10, 1, '','ASC', 132001, 2,
-------------------------------------------------------------------------------------------------*/

CREATE PROCEDURE [dbo].[st_rul_ApplicabilitySearchList_NonEmployee]
...
BEGIN
	...
	DECLARE @ERR_APPLICABILITY_SEARCH_LIST_NON_EMPLOYEE INT = 15132 -- Error occurred while fetching list of Non Employees for applicability allocation.
	...
	BEGIN TRY
		...
		/*Search non-employees which have applicability associated with @inp_nMapToTypeCodeId + @inp_nMapToId */	
		...
		SELECT @sSQL = @sSQL + ' LEFT JOIN	(
											...
											WHERE ADTemp.InsiderTypeCodeId = ' + CAST(@nUserTypeNonEmployeeCodeId AS VARCHAR) +
		...
		SELECT @sSQL = @sSQL + ' AND UI.UserTypeCodeId = ' + CAST(@nUserTypeNonEmployeeCodeId AS VARCHAR)
		...
		SELECT @sSQL = @sSQL + ' AND UI.StatusCodeId = ' + CAST(@nActiveUserStatusCodeId AS VARCHAR(20)) + ' '  /*Fetch only Active non-employees*/

		PRINT(@sSQL)
		EXEC(@sSQL)
		
		SELECT	UI.UserInfoId AS UserInfoId, 
		(ISNULL(UI.FirstName, '') + ' ' + ISNULL(UI.LastName, '')) AS rul_grd_15130 /*User's full name*/,
	...
	BEGIN CATCH
		...
		SET @out_nReturnValue	=  @ERR_APPLICABILITY_SEARCH_LIST_NON_EMPLOYEE
	END CATCH
END
