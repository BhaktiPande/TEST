IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_rul_ApplicabilitySearchList_Employee_OS')
DROP PROCEDURE [dbo].[st_rul_ApplicabilitySearchList_Employee_OS]
GO
SET ANSI_NULLS ON
GO
...
/*-------------------------------------------------------------------------------------------------
Description:	Procedure to list Applicability search list for applicability allocation - Employee (114132).
...
Usage:
EXEC st_rul_ApplicabilitySearchList_Employee_OS 10, 1, 'EmployeeId','ASC', 132001, 2,
-------------------------------------------------------------------------------------------------*/

CREATE PROCEDURE [dbo].[st_rul_ApplicabilitySearchList_Employee_OS]
	@inp_iPageSize			INT = 10
	,@inp_iPageNo			INT = 1
	,@inp_sSortField		VARCHAR(255)
	,@inp_sSortOrder		VARCHAR(5)
	,@inp_nGridTypeCodeId	INT		-- 114132 for Employee Insider , 114089 for Employee non insider.
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @sSQL = @sSQL + 								 ' SELECT UI.UserInfoId, UI.DepartmentId, UI.GradeId, UI.DesignationId, 
															  ...
															  INNER JOIN rul_ApplicabilityDetails_OS AD ON UI.UserTypeCodeId = AD.InsiderTypeCodeId AND UI.UserInfoId = AD.UserId 
															  INNER JOIN rul_ApplicabilityMaster_OS AM ON AM.ApplicabilityId = AD.ApplicabilityMstId  
															  WHERE 1=1 ' +
		...
		SELECT @sSQL = @sSQL + 								 ' SELECT UI.UserInfoId, UI.DepartmentId, UI.GradeId, UI.DesignationId, 
															  AD.UserId, AD.DepartmentCodeId, AD.GradeCodeId, AD.DesignationCodeId, AD.IncludeExcludeCodeId, 0 AS ExcludeIndicator  
															  FROM usr_UserInfo UI  
															  INNER JOIN rul_ApplicabilityDetails_OS AD ON UI.UserTypeCodeId = AD.InsiderTypeCodeId  
															  INNER JOIN rul_ApplicabilityMaster_OS AM ON AM.ApplicabilityId = AD.ApplicabilityMstId  
		...
		SELECT @sSQL = @sSQL + 								' UNION  
															  ...
															  FROM usr_UserInfo UI  
															  INNER JOIN rul_ApplicabilityDetails_OS AD ON UI.UserInfoId = AD.UserId  
															  INNER JOIN rul_ApplicabilityMaster_OS AM ON AM.ApplicabilityId = AD.ApplicabilityMstId  
															  WHERE 1=1 ' +
															' AND AD.InsiderTypeCodeId = ' + CAST(@nUserTypeEmployeeCodeId AS VARCHAR) + 
															' AND AD.AllEmployeeFlag = 0 AND AllInsiderFlag = 0 AND AllEmployeeInsiderFlag = 0 ' 
		...
		BEGIN 
			...
			ISNULL((CASE WHEN CdUICategory.DisplayCode IS NULL OR CdUICategory.DisplayCode = '' THEN CdUICategory.CodeName ELSE CdUICategory.DisplayCode END ),'') AS rul_grd_55342,--Category
			ISNULL((CASE WHEN CdUISubCategory.DisplayCode IS NULL OR CdUISubCategory.DisplayCode = '' THEN CdUISubCategory.CodeName ELSE CdUISubCategory.DisplayCode END ),'') AS rul_grd_55343,--Sub Category
			UR.RoleName AS rul_grd_55344, -- Role
		...
		BEGIN
			...
			CASE WHEN CdUIGrade.DisplayCode IS NULL OR CdUIGrade.DisplayCode = '' THEN CdUIGrade.CodeName ELSE CdUIGrade.DisplayCode END AS rul_grd_55333 /*Grade*/,
			CASE WHEN CdUIDesignation.DisplayCode IS NULL OR CdUIDesignation.DisplayCode = '' THEN CdUIDesignation.CodeName ELSE CdUIDesignation.DisplayCode END AS rul_grd_55334, /*Designation*/
			ISNULL((CASE WHEN CdUICategory.DisplayCode IS NULL OR CdUICategory.DisplayCode = '' THEN CdUICategory.CodeName ELSE CdUICategory.DisplayCode END ),'') AS rul_grd_55343,--Category
