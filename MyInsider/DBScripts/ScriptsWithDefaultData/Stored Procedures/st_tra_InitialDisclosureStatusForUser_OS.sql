IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_InitialDisclosureStatusForUser_OS')
DROP PROCEDURE [st_tra_InitialDisclosureStatusForUser_OS]
GO
...
CREATE PROCEDURE [dbo].[st_tra_InitialDisclosureStatusForUser_OS] --516
	 @inp_iUserInfoID				INT
	 ,@inp_iUserTypeID				INT
...
BEGIN
	...
	BEGIN TRY
		...
		IF EXISTS (SELECT * FROM eve_EventLog e WHERE EventCodeId IN (153052) AND UserInfoId = @iParentID AND e.EventDate>=@dtDateofBecomingInsider)
		BEGIN
			SET @nAllDocumentsCompleteFlag = @nStatusFlag_Complete
		END
		...
		BEGIN
			IF EXISTS (SELECT * FROM eve_EventLog e WHERE EventCodeId IN (153052) AND UserInfoId = @inp_iUserInfoID AND e.EventDate>=@dtDateofBecomingInsider)
			BEGIN
				...
				SELECT @nTransactionMasterID = MapToId, @ntmpEventCode = EventCodeId, @dttmpEventDate = EventDate 
					FROM eve_EventLog WHERE EventCodeId = 153052 and UserInfoId = @inp_iUserInfoID AND EventDate>=@dtDateofBecomingInsider 
			END
			ELSE
			BEGIN
				...
				IF EXISTS (SELECT * FROM eve_EventLog e WHERE EventCodeId IN (153053) AND UserInfoId = @inp_iUserInfoID AND e.EventDate>=@dtDateofBecomingInsider)
				BEGIN
		...
		ELSE
		BEGIN			
			select @nTradingPolicyId = TradingPolicyId from tra_TransactionMaster_OS where TransactionMasterId = @nTransactionMasterID
			...
			FROM rul_TradingPolicy_OS WHERE TradingPolicyId = @nTradingPolicyId
			...
			BEGIN
				...
				IF EXISTS (SELECT * FROM eve_EventLog WHERE UserInfoId = @inp_iUserInfoID AND EventCodeId = 153054 AND EventDate>=@dtDateofBecomingInsider)
				BEGIN
