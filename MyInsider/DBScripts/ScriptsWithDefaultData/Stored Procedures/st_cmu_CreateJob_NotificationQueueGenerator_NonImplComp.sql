...
Description:	This procedure when executed creats a job which will run on daily basis. 
				The job created runs continuously after every 15 mins and executes procedure "st_cmu_Job_NotificationQueueGenerator" 
...
/****** Object:  StoredProcedure [dbo].[st_cmu_CreateJob_NotificationQueueGenerator]    Script Date: 01/16/2017 18:09:16 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[st_cmu_CreateJob_NotificationQueueGenerator_NonImplComp]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[st_cmu_CreateJob_NotificationQueueGenerator_NonImplComp]
GO


CREATE PROCEDURE [dbo].[st_cmu_CreateJob_NotificationQueueGenerator_NonImplComp]
(
	...
	@inp_sLoginName				VARCHAR(50) = NULL,			-- SQL login name
...
BEGIN
	...
	BEGIN TRY
		...
		SET @sJobName = 'KPCSVigilanteComp_NonImpl' + CAST(@inp_nMasterDBCompanyId AS VARCHAR(10)) + '_' + @sDBName
		...
		EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@sJobName, 
				...
				@owner_login_name=@inp_sLoginName, 
				@job_id = @jobId OUTPUT
		IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
		...
		EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, 
				...
				EXECUTE @RC = [dbo].[st_cmu_CreateJob_NotificationQueueGenerator_NonImplComp] 
				GO', 
		...
		SET @sScheduleName = N'KPCSVigilanteComp_NonImpl'+ CAST(@inp_nMasterDBCompanyId AS VARCHAR(10)) +'_DailyEvery_15min'
