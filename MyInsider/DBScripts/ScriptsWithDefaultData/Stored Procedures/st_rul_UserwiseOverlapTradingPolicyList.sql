IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_rul_UserwiseOverlapTradingPolicyList')
DROP PROCEDURE [dbo].[st_rul_UserwiseOverlapTradingPolicyList]
GO
SET ANSI_NULLS ON
GO
...
GO
/*-------------------------------------------------------------------------------------------------
Description:	Procedure to list overlapping Trading Policies if any exist for the users defined for the trading policy being edited(denoted by @inp_nTradingPolicyId).
...
Usage:
EXEC st_rul_UserwiseOverlapTradingPolicyList 10, 1, '','ASC', 4
-------------------------------------------------------------------------------------------------*/


CREATE PROCEDURE [dbo].[st_rul_UserwiseOverlapTradingPolicyList]
...
BEGIN
	...
	BEGIN TRY
		...
		FROM rul_TradingPolicy WHERE TradingPolicyId = @inp_nTradingPolicyId
		...
		SELECT UserInfoId from vw_ApplicableTradingPolicyForUser 
		...
		INNER JOIN vw_ApplicableTradingPolicyForUser TPForUser ON CurrTPUsers.UserInfoId = TPForUser.UserInfoId
		...
		IF @inp_sSortField = 'rul_grd_15398' -- User ID
		BEGIN 
			SELECT @inp_sSortField = 'ISNULL(UI.EmployeeId, '''') ' 
		END
		
		IF @inp_sSortField = 'rul_grd_15399' -- UserType
		BEGIN 
		...
		IF @inp_sSortField = 'rul_grd_15400' -- Name (Username)
		BEGIN 
		...
		IF @inp_sSortField = 'rul_grd_15401' -- TradingPolicyName
		BEGIN 
			SELECT @inp_sSortField = 'TP.TradingPolicyName ' 
		END
		
		IF @inp_sSortField = 'rul_grd_15402' -- ApplicableFromDate
		BEGIN 
			SELECT @inp_sSortField = 'TP.ApplicableFromDate ' 
		END
		
		IF @inp_sSortField = 'rul_grd_15403' -- ApplicableToDate
		BEGIN 
		...
		SELECT @sSQL = @sSQL + ' INNER JOIN rul_TradingPolicy TP ON UOTP.TradingPolicyId = TP.TradingPolicyId '
		...
		SELECT ISNULL(UI.EmployeeId, '') AS rul_grd_15398 --User ID
		, ISNULL(CCUserType.DisplayCode, CCUserType.CodeName) AS rul_grd_15399 --UserType
		,CASE WHEN UI.UserTypeCodeId = @nUserTypeCodeIdCorporate THEN Company.CompanyName ELSE ISNULL(UI.FirstName, '') + ' ' + ISNULL(UI.LastName, '') END AS rul_grd_15400 --Name (User's name)
		, TP.TradingPolicyName AS rul_grd_15401 --Trading Policy Name
		, TP.ApplicableFromDate AS rul_grd_15402 --Applicable From Date
		, TP.ApplicableToDate AS rul_grd_15403 --Applicable To Date
		FROM #tmpList T 
		...
		INNER JOIN rul_TradingPolicy TP ON UOTP.TradingPolicyId = TP.TradingPolicyId
