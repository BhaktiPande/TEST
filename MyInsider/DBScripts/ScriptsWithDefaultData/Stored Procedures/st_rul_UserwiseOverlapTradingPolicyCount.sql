IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_rul_UserwiseOverlapTradingPolicyCount')
DROP PROCEDURE [dbo].[st_rul_UserwiseOverlapTradingPolicyCount]
GO
SET ANSI_NULLS ON
GO
...
Raghvendra		5-May-2016		Change done to first find the employees in the overlapping policies and then use the temp table in the below query for
								 optimizing  the query performance.
Raghvendra		07-Sep-2016		Changed the GETDATE() call with function dbo.uf_com_GetServerDate(). This function will return the date to be used as server date.
Usage:
EXEC st_rul_UserwiseOverlapTradingPolicyCount 10, 1, '','ASC', 4
-------------------------------------------------------------------------------------------------*/

CREATE PROCEDURE [dbo].[st_rul_UserwiseOverlapTradingPolicyCount]
...
BEGIN
	...
	BEGIN TRY
		...
		inner JOIN vw_ApplicableTradingPolicyForUser TPForUser ON TPForUser.MapToId = OTP.TradingPolicyId 
