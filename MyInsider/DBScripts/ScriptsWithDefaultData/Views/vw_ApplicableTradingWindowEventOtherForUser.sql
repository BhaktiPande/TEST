...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_ApplicableTradingWindowEventOtherForUser]'))
DROP VIEW [dbo].[vw_ApplicableTradingWindowEventOtherForUser]
GO

--select * from [dbo].[vw_ApplicableTradingWindowEventOtherForUser]

CREATE VIEW [dbo].[vw_ApplicableTradingWindowEventOtherForUser]
AS
...
(
-- For AllEmployeeFlag ********
SELECT 'AllEmployeeFlag' AS FromList, /*MAX(AD.ApplicabilityMstId) AS ApplicabilityMstId,*/ AD.ApplicabilityMstId AS ApplicabilityMstId, UF.UserInfoId , 0 AS IncludeFlag, TradingWindowEventId AS MapToId
...
JOIN vw_ApplicabilityMasterTradingWindowEventOther t ON AD.ApplicabilityMstId = t.ApplicabilityId
--GROUP BY UF.UserInfoId, TradingWindowEventId
UNION
SELECT 'AllEmployeeFlag', /*MAX(AD.ApplicabilityMstId) AS ApplicabilityMstId,*/ AD.ApplicabilityMstId AS ApplicabilityMstId, UR.UserInfoIdRelative, 0, TradingWindowEventId AS MapToId
FROM rul_ApplicabilityDetails AD JOIN usr_UserInfo UF ON AD.AllEmployeeFlag = 1 AND UF.UserTypeCodeId = 101003 AND UF.StatusCodeId = 102001
		AND (UF.DateOfBecomingInsider IS NULL OR UF.DateOfBecomingInsider <= dbo.uf_com_GetServerDate())
JOIN usr_UserRelation UR ON UF.UserInfoId = UR.UserInfoId
--JOIN rul_ApplicabilityMaster AM ON AD.ApplicabilityMstId = AM.ApplicabilityId AND AM.MapToTypeCodeId = 132001
JOIN vw_ApplicabilityMasterTradingWindowEventOther t ON AD.ApplicabilityMstId = t.ApplicabilityId
--GROUP BY UR.UserInfoIdRelative, TradingWindowEventId
UNION

-- For AllInsiderFlag ********
SELECT 'AllInsiderFlag', /*MAX(AD.ApplicabilityMstId) AS ApplicabilityMstId,*/ AD.ApplicabilityMstId AS ApplicabilityMstId, UF.UserInfoId, 0, TradingWindowEventId AS MapToId
...
--GROUP BY UF.UserInfoId, TradingWindowEventId
UNION
SELECT 'AllInsiderFlag', /*MAX(AD.ApplicabilityMstId) AS ApplicabilityMstId,*/ AD.ApplicabilityMstId AS ApplicabilityMstId, UR.UserInfoIdRelative, 0, TradingWindowEventId AS MapToId
FROM rul_ApplicabilityDetails AD JOIN usr_UserInfo UF ON AD.AllInsiderFlag = 1 AND 
	(/*UF.IsInsider = 1*/UF.DateOfBecomingInsider IS NOT NULL AND UF.DateOfBecomingInsider <= dbo.uf_com_GetServerDate() AND UF.StatusCodeId = 102001)-- AND (UF.DateOfSeparation IS NULL OR UF.DateOfSeparation >= CONVERT(DATETIME, CONVERT(VARCHAR(11), dbo.uf_com_GetServerDate()))))
JOIN usr_UserRelation UR ON UF.UserInfoId = UR.UserInfoId
--JOIN rul_ApplicabilityMaster AM ON AD.ApplicabilityMstId = AM.ApplicabilityId AND AM.MapToTypeCodeId = 132001
...
SELECT 'AllEmployeeInsiderFlag', /*MAX(AD.ApplicabilityMstId) AS ApplicabilityMstId,*/ AD.ApplicabilityMstId AS ApplicabilityMstId, UF.UserInfoId, 0, TradingWindowEventId AS MapToId
...
--GROUP BY UF.UserInfoId, TradingWindowEventId
UNION
SELECT 'AllEmployeeInsiderFlag', /*MAX(AD.ApplicabilityMstId) AS ApplicabilityMstId,*/ AD.ApplicabilityMstId AS ApplicabilityMstId, UR.UserInfoIdRelative, 0, TradingWindowEventId AS MapToId
FROM rul_ApplicabilityDetails AD JOIN usr_UserInfo UF ON AD.AllEmployeeInsiderFlag = 1 AND 
	(/*UF.IsInsider = 1*/UF.DateOfBecomingInsider IS NOT NULL AND UF.DateOfBecomingInsider <= dbo.uf_com_GetServerDate() AND UF.StatusCodeId = 102001)-- AND (UF.DateOfSeparation IS NULL OR UF.DateOfSeparation >= CONVERT(DATETIME, CONVERT(VARCHAR(11), dbo.uf_com_GetServerDate()))))
	 AND UF.UserTypeCodeId = 101003
JOIN usr_UserRelation UR ON UF.UserInfoId = UR.UserInfoId
...
SELECT 'AllNonEmployeeInsiderFlag', AD.ApplicabilityMstId AS ApplicabilityMstId, UF.UserInfoId, 0, TradingWindowEventId AS MapToId
...
SELECT 'AllCorporateInsiderFlag', AD.ApplicabilityMstId AS ApplicabilityMstId, UF.UserInfoId, 0, TradingWindowEventId AS MapToId
...
SELECT distinct 'InsiderTypeCodeId', /*MAX(AD.ApplicabilityMstId) AS ApplicabilityMstId,*/ AD.ApplicabilityMstId AS ApplicabilityMstId, UF.UserInfoId, 0, TradingWindowEventId AS MapToId
FROM rul_ApplicabilityDetails AD JOIN usr_UserInfo UF ON (/*UF.IsInsider = 1*/UF.DateOfBecomingInsider IS NOT NULL AND UF.DateOfBecomingInsider <= dbo.uf_com_GetServerDate() AND UF.StatusCodeId = 102001)-- AND (UF.DateOfSeparation IS NULL OR UF.DateOfSeparation >= CONVERT(DATETIME, CONVERT(VARCHAR(11), dbo.uf_com_GetServerDate())))) 
AND UF.UserTypeCodeId = AD.InsiderTypeCodeId
...
/*
UNION
SELECT distinct 'InsiderTypeCodeId', /*MAX(AD.ApplicabilityMstId) AS ApplicabilityMstId,*/ AD.ApplicabilityMstId AS ApplicabilityMstId, UR.UserInfoIdRelative, 0, TradingWindowEventId AS MapToId
FROM rul_ApplicabilityDetails AD JOIN usr_UserInfo UF ON (/*UF.IsInsider = 1*/UF.DateOfBecomingInsider IS NOT NULL AND UF.DateOfBecomingInsider <= dbo.uf_com_GetServerDate() AND UF.StatusCodeId = 102001)-- AND (UF.DateOfSeparation IS NULL OR UF.DateOfSeparation >= CONVERT(DATETIME, CONVERT(VARCHAR(11), dbo.uf_com_GetServerDate())))) 
AND UF.UserTypeCodeId = AD.InsiderTypeCodeId
JOIN usr_UserRelation UR ON UF.UserInfoId = UR.UserInfoId
JOIN usr_UserRole UROL ON UF.UserInfoID = UROL.UserInfoID
...
UNION
-- Exclude set ***********
SELECT distinct 'Exclude', /*MAX(AD.ApplicabilityMstId) AS ApplicabilityMstId,*/ AD.ApplicabilityMstId AS ApplicabilityMstId, UF.UserInfoId, 1, TradingWindowEventId AS MapToId
FROM rul_ApplicabilityDetails AD JOIN usr_UserInfo UF ON (/*UF.IsInsider = 1 AND UF.DateOfBecomingInsider IS NOT NULL AND UF.DateOfBecomingInsider <= dbo.uf_com_GetServerDate()*/  UF.StatusCodeId = 102001)-- AND (UF.DateOfSeparation IS NULL OR UF.DateOfSeparation >= CONVERT(DATETIME, CONVERT(VARCHAR(11), dbo.uf_com_GetServerDate())))) 
AND UF.UserTypeCodeId = AD.InsiderTypeCodeId
--JOIN rul_ApplicabilityMaster AM ON AD.ApplicabilityMstId = AM.ApplicabilityId AND AM.MapToTypeCodeId = 132001
JOIN vw_ApplicabilityMasterTradingWindowEventOther t ON AD.ApplicabilityMstId = t.ApplicabilityId
WHERE 1 = 1
...
--GROUP BY UF.UserInfoId, TradingWindowEventId
UNION
SELECT distinct 'Exclude', /*MAX(AD.ApplicabilityMstId) AS ApplicabilityMstId,*/ AD.ApplicabilityMstId AS ApplicabilityMstId, UR.UserInfoIdRelative, 1, TradingWindowEventId AS MapToId
FROM rul_ApplicabilityDetails AD JOIN usr_UserInfo UF ON (/*UF.IsInsider = 1 AND UF.DateOfBecomingInsider IS NOT NULL AND UF.DateOfBecomingInsider <= dbo.uf_com_GetServerDate() */  UF.StatusCodeId = 102001)-- AND (UF.DateOfSeparation IS NULL OR UF.DateOfSeparation >= CONVERT(DATETIME, CONVERT(VARCHAR(11), dbo.uf_com_GetServerDate())))) 
AND UF.UserTypeCodeId = AD.InsiderTypeCodeId
JOIN usr_UserRelation UR ON UF.UserInfoId = UR.UserInfoId
--JOIN rul_ApplicabilityMaster AM ON AD.ApplicabilityMstId = AM.ApplicabilityId AND AM.MapToTypeCodeId = 132001
