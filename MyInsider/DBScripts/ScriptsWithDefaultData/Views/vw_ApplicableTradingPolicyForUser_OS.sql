...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_ApplicableTradingPolicyForUser_OS]'))
DROP VIEW [dbo].[vw_ApplicableTradingPolicyForUser_OS]
GO

CREATE VIEW [dbo].[vw_ApplicableTradingPolicyForUser_OS]
AS
...
FROM rul_ApplicabilityDetails_OS AD JOIN usr_UserInfo UF ON AD.AllEmployeeFlag = 1 AND UF.UserTypeCodeId = 101003 AND UF.StatusCodeId = 102001
		AND (UF.DateOfBecomingInsider IS NULL OR UF.DateOfBecomingInsider > dbo.uf_com_GetServerDate())
JOIN vw_ApplicabilityMasterCurrentTradingPolicy_OS vwAppMaster ON vwAppMaster.ApplicabilityId = AD.ApplicabilityMstId
...
FROM rul_ApplicabilityDetails_OS AD JOIN usr_UserInfo UF ON AD.AllInsiderFlag = 1 AND 
...
FROM rul_ApplicabilityDetails_OS AD JOIN usr_UserInfo UF ON AD.AllEmployeeInsiderFlag = 1 AND 
	(/*UF.IsInsider = 1*/UF.DateOfBecomingInsider IS NOT NULL AND UF.DateOfBecomingInsider <= dbo.uf_com_GetServerDate() AND UF.StatusCodeId = 102001)--AND (UF.DateOfSeparation IS NULL OR UF.DateOfSeparation >= CONVERT(DATETIME, CONVERT(VARCHAR(11), dbo.uf_com_GetServerDate()))))
...
FROM rul_ApplicabilityDetails_OS AD JOIN usr_UserInfo UF ON AD.AllNonEmployee = 1 AND 
	(/*UF.IsInsider = 1*/UF.DateOfBecomingInsider IS NOT NULL AND UF.DateOfBecomingInsider <= dbo.uf_com_GetServerDate() AND UF.StatusCodeId = 102001)--AND (UF.DateOfSeparation IS NULL OR UF.DateOfSeparation >= CONVERT(DATETIME, CONVERT(VARCHAR(11), dbo.uf_com_GetServerDate()))))
...
FROM rul_ApplicabilityDetails_OS AD JOIN usr_UserInfo UF ON AD.AllCorporateEmployees = 1 AND 
	(/*UF.IsInsider = 1*/UF.DateOfBecomingInsider IS NOT NULL AND UF.DateOfBecomingInsider <= dbo.uf_com_GetServerDate() AND UF.StatusCodeId = 102001)--AND (UF.DateOfSeparation IS NULL OR UF.DateOfSeparation >= CONVERT(DATETIME, CONVERT(VARCHAR(11), dbo.uf_com_GetServerDate()))))
...
FROM rul_ApplicabilityDetails_OS AD JOIN usr_UserInfo UF ON /*UF.IsInsider = 1 AND */ UF.DateOfBecomingInsider IS NOT NULL AND UF.DateOfBecomingInsider <= dbo.uf_com_GetServerDate() AND UF.UserTypeCodeId = AD.InsiderTypeCodeId
JOIN vw_ApplicabilityMasterCurrentTradingPolicy_OS vwAppMaster ON vwAppMaster.ApplicabilityId = AD.ApplicabilityMstId
...
FROM rul_ApplicabilityDetails_OS AD JOIN usr_UserInfo UF ON /*UF.IsInsider = 1 AND */ (UF.DateOfBecomingInsider IS  NULL OR UF.DateOfBecomingInsider > dbo.uf_com_GetServerDate()) AND UF.UserTypeCodeId = AD.InsiderTypeCodeId
...
FROM rul_ApplicabilityDetails_OS AD JOIN usr_UserInfo UF ON /*UF.IsInsider = 1 AND */ UF.UserTypeCodeId = AD.InsiderTypeCodeId  AND UF.StatusCodeId = 102001
JOIN vw_ApplicabilityMasterCurrentTradingPolicy_OS vwAppMaster ON vwAppMaster.ApplicabilityId = AD.ApplicabilityMstId
WHERE 1 = 1
