...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_ApplicabilityMasterCurrentTradingPolicy]'))
DROP VIEW [dbo].[vw_ApplicabilityMasterCurrentTradingPolicy]
GO

CREATE VIEW [dbo].[vw_ApplicabilityMasterCurrentTradingPolicy]
AS
...
(
 ...
 FROM (
       SELECT TP.TradingPolicyId AS TradingPolicyId, ISNULL(TP.TradingPolicyParentId,TradingPolicyId) AS TradingPolicyParentId FROM rul_TradingPolicy TP
	   WHERE 
...
SELECT AM.* FROM rul_ApplicabilityMaster AM JOIN
(SELECT MapToTypeCodeId, MapToId, MAX(ApplicabilityId) AS ApplicabilityId
FROM rul_ApplicabilityMaster AM JOIN rul_TradingPolicy TP ON MapToTypeCodeId = 132002 AND AM.MapToId = TP.TradingPolicyId
