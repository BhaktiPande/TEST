...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vw_ApplicableRLForUser]'))
DROP VIEW [dbo].[vw_ApplicableRLForUser]
GO

CREATE VIEW [dbo].[vw_ApplicableRLForUser]
AS

SELECT MAX(ApplicabilityMstId) ApplicabilityMstId, MapToId,UserInfoId --, MAX(IncludeFlag) AS IncludeFlag
FROM
(
-- For AllEmployeeFlag ********
...
AND (vwAppMaster.UserId IS NULL OR (UF.UserInfoId = vwAppMaster.UserId AND IncludeExcludeCodeId = 150001))
...
JOIN vw_ApplicabilityMasterCurrentRL vwAppMaster ON vwAppMaster.ApplicabilityId = AD.ApplicabilityMstId
WHERE 1 = 1
AND (UF.UserInfoId = vwAppMaster.UserId AND IncludeExcludeCodeId = 150002)
GROUP BY UF.UserInfoId, MapToId
UNION
