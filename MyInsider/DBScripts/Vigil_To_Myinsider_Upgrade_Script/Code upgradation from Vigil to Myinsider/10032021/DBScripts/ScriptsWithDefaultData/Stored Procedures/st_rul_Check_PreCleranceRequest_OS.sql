IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_rul_Check_PreCleranceRequest_OS')
DROP PROCEDURE [dbo].[st_rul_Check_PreCleranceRequest_OS]
GO
SET ANSI_NULLS ON
GO
...
GO
/*-------------------------------------------------------------------------------------------------
Description:	Check the PreClearance Request

Created by:		Rajashri
Created on:		29-July-2020
-------------------------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[st_rul_Check_PreCleranceRequest_OS]
...
BEGIN
	...
	BEGIN TRY
		...
		IF @out_sSQLErrMessage IS NULL
	    	...
	    	DELETE FROM rul_TradingPolicySecuritywiseLimits_OS 
			WHERE TradingPolicyId = @inp_iTradingPolicyId AND 
			MapToTypeCodeId = @nMapToTypePreclearance			
			
			--PreClrTradesApprovalReq Flag  is zero then insert the rescored
			...
			DELETE FROM rul_TradingPolicyForTransactionSecurity_OS
			WHERE TradingPolicyId = @inp_iTradingPolicyId AND MapToTypeCodeId = 132015
			...
			INSERT INTO rul_TradingPolicyForTransactionSecurity_OS 
				...
				union 
				SELECT @inp_iTradingPolicyId, 132015 as MapToTypeCodeId, 143006 as 	TransactionModeCodeId ,139001 as	SecurityTypeCodeId
				union 
				SELECT @inp_iTradingPolicyId, 132015 as MapToTypeCodeId, 143007 as 	TransactionModeCodeId ,139001 as	SecurityTypeCodeId
				union
				SELECT @inp_iTradingPolicyId, 132015 as MapToTypeCodeId, 143008 as 	TransactionModeCodeId ,139001 as	SecurityTypeCodeId
