...
BEGIN
	...
	DECLARE @nContinousDisclosureType											INT,
			...
			DECLARE @nYear varchar(15)= YEAR(getdate());
	...
	BEGIN TRY
		...
		*/
		CREATE TABLE #tmpUsers(Id INT IDENTITY(1,1), IsAddButtonRow INT DEFAULT 0,UserInfoId INT, TransactionMasterID INT,TradingPolicyId INT, PreClearanceRequestID INT,PreclearanceStatus INT,
						       ...
						       TradedQty DECIMAL(10,0),DisplayNumber INT, TMIdForOrder BIGINT,IsEnterAndUploadEvent INT DEFAULT 0)
			...
			*/
			...
			where UserInfoId = @inp_iUserInfoID AND DisclosureTypeCodeId = @nContinousDisclosureType AND TransactionStatusCodeId = 148002 
		...
		*/
		...
		SELECT DISTINCT CASE WHEN TW.WindowCloseDate BETWEEN EL.EventDate AND (select dbo.uf_tra_GetNextTradingDateOrNoOfDaysWithWinCloseDate(EL.EventDate,PreClrApprovalValidityLimit,null,0,1,0,116001,NULL,NULL)) THEN TW.WindowCloseDate END AS ValidityDate ,
		...
		FROM rul_ApplicabilityDetails AD 
			...
			LEFT JOIN #tmpUsers T ON T.UserInfoId=AD.UserId
			LEFT JOIN eve_EventLog EL ON EL.MapToId=T.PreClearanceRequestID
			JOIN rul_TradingPolicy TP ON T.TradingPolicyId = TP.TradingPolicyId  
			WHERE AD.UserId=@inp_iUserInfoID AND AM.MapToTypeCodeId=@TradingWindowOtherMapToId 
		...
		SELECT DISTINCT EL.MapToId,EL.EventDate,PreClrApprovalValidityLimit FROM #tmpUsers T 
			...
			UPDATE T
			SET PreclearanceValidTill = (select dbo.uf_tra_GetNextTradingDateOrNoOfDaysWithWinCloseDate(EL.EventDate,PreClrApprovalValidityLimit,null,0,1,0,116001,TEMP.EventType,TEMP.WindowCloseDate))  --DATEADD(DAY,TP.PreClrApprovalValidityLimit+PreClrCOApprovalLimit,PR.CreatedOn) < dbo.uf_com_GetServerDate() THEN NULL 
											 --ELSE (select dbo.uf_tra_GetNextTradingDateOrNoOfDaysWithWinCloseDate(PR.CreatedOn,TP.PreClrApprovalValidityLimit+PreClrCOApprovalLimit,null,1,1,0,116001)) END 
			FROM #tmpUsers T 
			JOIN eve_EventLog EL ON t.UserInfoId = EL.UserInfoId 
			JOIN tra_PreclearanceRequest PR ON T.PreClearanceRequestID = PR.PreclearanceRequestId
			JOIN rul_TradingPolicy TP ON T.TradingPolicyId = TP.TradingPolicyId 
			LEFT JOIN #TempParameters TEMP ON TEMP.MapToId=T.PreClearanceRequestID
			WHERE   t.PreClearanceRequestID = EL.MapToId 
				 AND EL.MapToTypeCodeId = @nPreclearanceRequestMapToType 
				 AND EL.EventCodeId IN(@nEventPreclearanceApproved)
		...
		*/		
		IF @nEventType=@TradingWindowEventTypeFinancialYear
		BEGIN
			UPDATE T
	  ...
	  --select * from #tmpUsers
	   SELECT Temp.TransactionMasterID,
			    ...
			    code2.CodeName AS dis_grd_17019,
			    code1.CodeName AS dis_grd_17018 ,
			   ...
			   ,CASE WHEN Temp.HardcopySubmissionwithin < 0 THEN ABS(HardcopySubmissionwithin) ELSE Temp.HardcopySubmissionwithin END HardcopySubmissionwithin
				...
				,CASE WHEN DisplayNumber = 1  THEN ROUND(PR.SecuritiesToBeTradedQty,0) ELSE null END AS dis_grd_17350
				,IsAddButtonRow 
				,Temp.UserInfoId
