...
IF(@inp_iStockExchangeSubmissionStatus=154008)
BEGIN
...
ELSE If(@inp_iStockExchangeSubmissionStatus=154002)
BEGIN
		INSERT INTO #tmpGroupWiseUser(TransactionMasterId) (	   
...
INSERT INTO #tmpUsers(UserInfoId,--EmployeeID,EmployeeName,
		...
		*/
		...
		SELECT DISTINCT EL.MapToId,EL.EventDate,PreClrApprovalValidityLimit FROM #tmpUsers T 
			...
			BEGIN
				...
				SELECT @nWindowCloseDate = tw.WindowCloseDate 
				FROM rul_TradingWindowEvent tw 
				WHERE EventTypeCodeId=@TradingWindowEventTypeFinancialYear
				AND MONTH(tw.WindowCloseDate)=MONTH(@nEventDate) AND YEAR(tw.WindowCloseDate)=YEAR(@nEventDate)
			...
			UPDATE T
			SET PreclearanceValidTill = (select dbo.uf_tra_GetNextTradingDateOrNoOfDaysWithWinCloseDate(EL.EventDate,PreClrApprovalValidityLimit,null,0,1,0,116001,TEMP.EventType,TEMP.WindowCloseDate))
			FROM #tmpUsers T 
			JOIN eve_EventLog EL ON t.UserInfoId = EL.UserInfoId 
			JOIN tra_PreclearanceRequest PR ON T.PreClearanceRequestID = PR.PreclearanceRequestId
			JOIN rul_TradingPolicy TP ON T.TradingPolicyId = TP.TradingPolicyId 
	...
	--	OR (SoftCopyToBeSubmitFlag =  1 AND SoftCopySubmissionFlag = 1 AND HardCopySubmissionFlag = 2))
	  ...
	  -- Based on search parameters, insert only the Primary Index Field in the temporary table.
		...
		--IF @inp_sSortField  = 'dis_grd_50597'
		--BEGIN 
		...
		BEGIN 
			SELECT @inp_sSortField = 'Temp.ContinuousDisclosureStatus ,Temp.ContinuousDisclosureSubmissionDate'
		END
		...
		BEGIN
				SELECT @sSQL = @sSQL + ' AND UF.PAN like ''%' + CONVERT(VARCHAR,@inp_sPANNumber) + '%'''
		END
		...
		IF(@UpdateInProgressStatus=154008 OR @inp_iStockExchangeSubmissionStatus IS NULL)
		BEGIN
		...
		--UPDATE tmp
		--SET HardCopySubmissionCOToExchangeFlag=3
	   	--FROM #tmpUsers tmp WHERE  EXISTS
  --(SELECT *
  -- FROM   #tmpGroupWiseUser1 tmp1  
   --WHERE  tmp1.TransactionMasterId= tmp.TransactionMasterId)  
		END
	   ...
	   SELECT Temp.TransactionMasterID,
			    ...
			    code2.CodeName AS dis_grd_17260,
			    code1.CodeName AS dis_grd_17259 ,
				...
				, Row_Number() OVER (ORDER BY (SELECT T.RowNumber) Desc)  AS RowNum 
		FROM #tmpList T 
		...
		ORDER BY T.RowNumber asc, RowNum desc				  
