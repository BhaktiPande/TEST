...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			IF EXISTS(SELECT 1 FROM usr_UserInfo WHERE EmailId = @inp_sEmailId AND UserTypeCodeId = @nUserType_CO AND UserInfoId != @inp_iUserInfoId)
				BEGIN
		...
		BEGIN
			IF EXISTS(SELECT 1 FROM usr_UserInfo WHERE EmailId = @inp_sEmailId AND UserTypeCodeId = @nUserType_Employee AND UserInfoId != @inp_iUserInfoId)
				BEGIN
					SET @out_nReturnValue = @ERR_EMAILID_EXISTS
					RETURN (@out_nReturnValue)	
				END
		...
		BEGIN
			IF(
					EXISTS(SELECT UserInfoId FROM usr_UserInfo WHERE EmailId = @inp_sEmailId 
							AND UserTypeCodeId <> @nUserType_Relative AND (@inp_iParentId IS NULL OR @inp_iParentId <> UserInfoId)
							AND (@inp_iUserInfoId IS NULL OR @inp_iUserInfoId = 0 OR UserInfoId != @inp_iUserInfoId ) )
					OR
					EXISTS(SELECT u.UserInfoId FROM usr_UserInfo u 
							...
							AND (ur.UserInfoId <> @inp_iParentId) )
				)
				BEGIN
					SET @out_nReturnValue = @ERR_EMAILID_EXISTS
					RETURN (@out_nReturnValue)
		...
		BEGIN
			IF(
				EXISTS(SELECT UserInfoId FROM usr_UserInfo WHERE (MobileNumber = @MobNumber_WithCountyCode OR MobileNumber = @MobNumber_WithoutCountyCode)
						AND UserTypeCodeId <> @nUserType_Relative AND UserInfoId NOT IN (@inp_iUserInfoId) AND (@inp_iParentId IS NULL OR @inp_iParentId <> UserInfoId)
						AND (@inp_iUserInfoId IS NULL OR @inp_iUserInfoId = 0 OR UserInfoId != @inp_iUserInfoId ) )
				OR
				EXISTS(SELECT u.UserInfoId FROM usr_UserInfo u 
						JOIN usr_UserRelation ur ON u.UserInfoId = ur.UserInfoIdRelative 
						WHERE (MobileNumber = @MobNumber_WithCountyCode OR MobileNumber = @MobNumber_WithoutCountyCode) AND UserTypeCodeId = @nUserType_Relative AND u.UserInfoId NOT IN (@inp_iUserInfoId)
						AND (@inp_iUserInfoId IS NULL OR @inp_iUserInfoId = 0 OR u.UserInfoId != @inp_iUserInfoId ) 
						AND (ur.UserInfoId <> @inp_iParentId) )
			)
			BEGIN
		...
		BEGIN
			...
			EXEC @nTmpRetVal = st_usr_UserInfoSave_Employee
					...
					@inp_iLoggedInUserId, @inp_sLoginID, @inp_sPassword, @inp_iIsInsider,@inp_sDIN, @out_nReturnValue OUTPUT, @out_nSQLErrCode OUTPUT, @out_sSQLErrMessage OUTPUT
		...
		BEGIN
			...
			EXEC @nTmpRetVal = st_usr_UserInfoSave_Corporate
					...
					@inp_iLoggedInUserId, @inp_sLoginID, @inp_sPassword,@inp_sCIN,@inp_sCategoryText, @inp_sSubCategoryText,
					@out_nReturnValue OUTPUT, @out_nSQLErrCode OUTPUT, @out_sSQLErrMessage OUTPUT
		...
		BEGIN
			EXEC @nTmpRetVal = st_usr_UserInfoSave_NonEmployee
					...
					@inp_iIsInsider, @inp_sLoginID, @inp_sPassword,@inp_sDIN,
					@out_nReturnValue OUTPUT, @out_nSQLErrCode OUTPUT, @out_sSQLErrMessage OUTPUT
		...
		BEGIN
			...
			EXEC @nTmpRetVal = st_usr_UserInfoSave_Relative
					...
					@inp_sPAN, @inp_iStatusCodeId, @inp_iLoggedInUserId,
					@out_nReturnValue OUTPUT, @out_nSQLErrCode OUTPUT, @out_sSQLErrMessage OUTPUT
		END
