...
namespace ESOP.SSO.Library
{
    ...
    public class SAMLResponse : IDisposable
    {
        ...
        private string UPNValue { get; set; }
        /// <summary>
                    ...
                    for (int i = 0; i < ds.Tables.Count; i++)
                    {
                        switch (ds.Tables[i].ToString().ToUpper())
                        {
                            ...
                            case "X509DATA": // Here Getting NameID value
                                ...
                                break;
                            case "SUBJECT": // Here Getting NameID value
                                GetNameIdFromSubject(ds.Tables[i]);
                                break;
        ...
        private void GetNameIdFromSubject(DataTable dataTable)
        {
            try
            {
                if (!dataTable.Equals(null) && dataTable.Rows.Count > 0)
                {
                    ...
                    NameId = dataTable.Rows[0]["NameID"].ToString();
                    if (NameId.Contains('@'))
                    {
                        Email = NameId;
                    }
        ...
        private void FillSsoProperty()
        {
            try
            {
                ...
                SsoProperty.UPNFromSAMALResponse = UPNValue;
                SsoProperty.IsValidateSSO = Convert.ToBoolean(IsSSOValidate);
        ...
        public DataTable GetSSODetailsByIDPUrl(string idpSpUrl)
        {
            SetCustomMessage(false, "GetSSODetailsByIDPUrl from ESOP Library.\n");
            string _sqlWhere = null;
            DataTable dataTableSSOCompanyList = new DataTable();
            dataTableSSOCompanyList = Generic.Instance().GetAllSsoCompanyList();
            if (dataTableSSOCompanyList != null && dataTableSSOCompanyList.Rows.Count > 0)
            {
                _sqlWhere = " IDP_SP_URL = '" + idpSpUrl.Trim() + "'";
            ...
            return dataTableSSOCompanyDetails;
        }
        public DataTable GetSSODetailsByCompanyName(string companyName)
        {
            SetCustomMessage(false, "GetSSODetailsByIDPUrl from ESOP Library.\n");
            string _sqlWhere = null;
            DataTable dataTableSSOCompanyList = new DataTable();
            try
            {
                ...
                if (dataTableSSOCompanyList != null && dataTableSSOCompanyList.Rows.Count > 0)
                {
                    _sqlWhere = " CompanyName = '" + companyName.Trim() + "'";
                    dataTableSSOCompanyDetails = new DataTable();
                    if (dataTableSSOCompanyList.Rows.Count > 0 && dataTableSSOCompanyList != null)
                    {
                        dataTableSSOCompanyDetails = dataTableSSOCompanyList.Select(_sqlWhere).CopyToDataTable();
                        SsoProperty = new SSO();
                        SsoProperty.CompanyName = dataTableSSOCompanyDetails.Rows[0]["CompanyName"].ToString();
                        SsoProperty.IDP_SP_URL = dataTableSSOCompanyDetails.Rows[0]["IDP_SP_URL"].ToString();
                        SsoProperty.IssuerURL = dataTableSSOCompanyDetails.Rows[0]["IssuerURL"].ToString();
                        SsoProperty.AssertionConsumerServiceURL = dataTableSSOCompanyDetails.Rows[0]["AssertionConsumerServiceURL"].ToString();
                        SsoProperty.RelayState = dataTableSSOCompanyDetails.Rows[0]["RelayState"].ToString();
                        SsoProperty.Certificate = dataTableSSOCompanyDetails.Rows[0]["Certificate"].ToString();
                        SsoProperty.DestinationURL = dataTableSSOCompanyDetails.Rows[0]["DestinationURL"].ToString();
                    }
                    else
                    {
                        dataTableSSOCompanyDetails = null;
                    }
        ...
        private void FillSSOAllDetails(DataRow dataRow)
        {
            try
            {
                ...
                SsoProperty.IsValidateSSO = IsSSOValidate;
                WriteMessage();
            }
            catch (Exception ex)
            {
        ...
        private void MatchingKeyWordOfSSoAttributeName(string attributeName, string attributevalue)
        {
            ...
            if (!iSAttributeNameMatched.Equals(true))
            {
                ...
                if (!iSAttributeNameMatched.Equals(true))
                {
                    ...
                    if (!iSAttributeNameMatched.Equals(true))
                    {
                        iSAttributeNameMatched = IsSSOUPNAttributeMatched(attributeName, attributevalue);
                    }
                }
            }
        }

        private bool IsSSOUPNAttributeMatched(string attributeName, string attributevalue)
        {
            string[] upnKeyWord = new string[] { "UPN", "UPNID", "upn" };
            bool iSUPNAttributeMatched = IsContainsAttributeKey(attributeName, upnKeyWord);
            if (iSUPNAttributeMatched.Equals(true))
            {
                UPNValue = attributevalue;
            }
            return iSUPNAttributeMatched;
        }
