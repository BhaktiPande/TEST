$(document).ready(function () {
    ...
    */
    $(document).delegate('#TradingPolicyForCodeId', 'change', function (event) {
        ...
        //If Policy is Employee Type
        if (nTradingPolicyForCodeId == $('#TradingPolicyEmployeeType').val()) {
            // Set by default 'Multiple Transaction Trade above' is selected in COntinuous Disclosure section for Employee type policy.
            $(':radio[name="StExSingMultiTransTradeFlagCodeId"]').prop("checked", $('#MultipleTransactionTrade').val());
            var nStExSingMultiTransTradeFlagCodeId = $(':radio[name="StExSingMultiTransTradeFlagCodeId"]').filter(':checked').val();
            if (nStExSingMultiTransTradeFlagCodeId == 136002) {
                $.ajax({
            ...
            $.ajax({
                url: $('#ContraTradeSecuirtyMapping').val(),
                type: 'post',
                headers: getRVToken(),
                cache: false,
                data: values,
   ...
   $(document).delegate('#StExMultiTradeFreq', 'change', function (event) {
       var URL = $('#StExMultiTradeFreqURL').val();
       var a = $('#StExMultiTradeFreq').val();
       var values = $("#frmTradingPolicy").serializeArray();
       values.push({
           name: "acid",
           value: $("#TradingPolicyUserAction").val()
       ...
       if (a == $('#OccurrenceFrequencyYearly').val()) {
           $('#DivYearType').html("");
           $.ajax({
               url: $("#StExMultiTradeFreqURL").val(),
               type: 'post',
               headers: getRVToken(),
               cache: false,
               data: values,
               success: function (result) {
                   $('#DivYearType').html(result);
               },
               error: function (result) {
               }
           });
   ...
   $("#BasicInfo").click(function () {
       ...
       var values = $("#frmTradingPolicy").serializeArray();
       //alert(JSON.stringify(PreclearanceSecuritywisedata));
       ...
       if ($('#TradingPolicyId').val() > 0 && $('#CountUserAndOverlapTradingPolicy').val() > 0 ) {
           $.ajax({
               ...
               type: 'post',
               headers: getRVToken(),
               data: { 'TradingPolicyId': $('#TradingPolicyId').val(), 'ShowFrom': 'TradingPolicy' },//values,
