...
{
    public class NotificationDAL_OS:IDisposable
    {
        ...
        public NotificationDTO GetDetails(string sConnectionString, int inp_iNotificationQueueId)
        {
            ...
            try
            {
                ...
                using (var db = new PetaPoco.Database(sConnectionString, "System.Data.SqlClient") { EnableAutoSelect = false })
                {
                    ...
                    {
                        res = db.Query<NotificationDTO>("exec st_cmu_NotificationDetails_OS @inp_iNotificationQueueId,@out_nReturnValue OUTPUT,@out_nSQLErrCode OUTPUT,@out_sSQLErrMessage OUTPUT",
                        new
                        {
