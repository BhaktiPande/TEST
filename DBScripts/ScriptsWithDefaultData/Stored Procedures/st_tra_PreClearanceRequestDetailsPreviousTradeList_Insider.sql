IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_PreClearanceRequestDetailsPreviousTradeList_Insider')
DROP PROCEDURE [dbo].[st_tra_PreClearanceRequestDetailsPreviousTradeList_Insider]
GO
SET ANSI_NULLS ON
GO
...
Parag			28-Aug-2015     Made change to show "Pending" status text when pre clearance status is "Confirmed"
...
Parag			21-Sep-2015		Made change to show period wise total 
Parag			22-Dec-2015		Made chagne to fix issue of pre clearance quantity requested not shown
...
Usage:
EXEC st_tra_PreClearanceRequestDetailsPreviousTradeList_Insider 10, 1, '','ASC', 52
-------------------------------------------------------------------------------------------------*/

CREATE PROCEDURE [dbo].[st_tra_PreClearanceRequestDetailsPreviousTradeList_Insider]
	@inp_iPageSize			INT = 10
	,@inp_iPageNo			INT = 1
	,@inp_sSortField		VARCHAR(255)
	,@inp_sSortOrder		VARCHAR(5)
	,@inp_iUserInfoID		INT						-- UserID for whom the previous preclearance, PNT and trade details are to be listed.
...
BEGIN
	...
	DECLARE @ERR_PRECLEARANCE_REQUEST_DETAILS_LIST_INSIDER INT = 17378
	...
	BEGIN TRY
		...
		--Fetch the contra trade days from ongoing trading policy for input userID
		SELECT 
			@nContraTradeDays = MAX(ISNULL(TP.GenContraTradeNotAllowedLimit,0))
		FROM 
			vw_ApplicableTradingPolicyForUser TPForUser 
			JOIN rul_TradingPolicy TP ON TPForUser.MapToId = TP.TradingPolicyId
		WHERE UserInfoId = @inp_iUserInfoID

		
		SELECT @nContraTradeDays = -@nContraTradeDays
		SELECT @dtLastContraTradeDate = DATEADD(day, @nContraTradeDays, dbo.uf_com_GetServerDate())
		SELECT @dtCurrentPreclearanceDATE = dbo.uf_com_GetServerDate()
		
		print @nContraTradeDays
		--------------------------------
		--SET @nContraTradeDays = -10
		--------------------------------
		...
		FROM 
			...
			LEFT JOIN tra_PreclearanceRequest PCR ON TM.PreclearanceRequestId = PCR.PreclearanceRequestId
		...
		WHERE 
			...
			AND (EL.EventDate IS NULL OR (EL.EventCodeId IS NOT NULL AND (CONVERT(date, EL.EventDate) >= @dtLastContraTradeDate AND convert(date, EL.EventDate) <= @dtCurrentPreclearanceDATE)))
		...
		BEGIN
			...
			BEGIN
				...
				UPDATE #tmpUsers SET PreclearanceValue = tmp2.SecuritiesToBeTradedValue, PreclearanceQty = tmp2.SecuritiesToBeTradedQty
				FROM (
					SELECT tmp.mId, PCR.PreclearanceRequestId, PCR.SecuritiesToBeTradedQty, PCR.SecuritiesToBeTradedValue FROM tra_PreclearanceRequest PCR 
					JOIN (
						SELECT MAX(Id) AS mId, t.PreClearanceRequestID FROM #tmpUsers t 
						WHERE 
							t.PreClearanceRequestID IS NOT NULL AND 
				...
				SELECT @nTotalPreclearanceValue = SUM(U.PreclearanceValue), @nTotalPreclearanceQuantity = SUM(U.PreclearanceQty) 
				FROM #tmpUsers U 
				WHERE CONVERT(date,U.SortDateField) >= CONVERT(date,@dtPEStartDate) AND CONVERT(date,U.SortDateField) <= CONVERT(date,@dtPEEndDate)
		...
		SELECT	
			TmpPreClearance.DisplayName AS dis_grd_17369, --display text , 
			TmpPreClearance.PreClearanceDate AS dis_grd_17370, --Date : PreClearanceDate, 
			TmpPreClearance.DateOfAcquisition AS dis_grd_17388, -- Date of Acquisition
			ccTransactionTypeCode.CodeName AS dis_grd_17371, --Transaction Type : PreClearanceTransactionTypeCode, 
			ccSecuritiesCode.CodeName AS dis_grd_17372, --Type of Security : PreClearanceSecuritiesCode, 
			TmpPreClearance.PreclearanceQty AS dis_grd_17373, --No. of Securities : PreclearanceQty, 
			TmpPreClearance.PreclearanceValue AS dis_grd_17374, --Pre-Clearance Value : PreclearanceValue, 
			TmpPreClearance.TradedQty AS dis_grd_17375, -- TradedQty
			TmpPreClearance.TradeValue AS dis_grd_17376, --TradeValue
			CASE 
				WHEN TmpPreClearance.PreclearanceStatus = @nPreClearanceConfirmedCodeId THEN @sPendingText
				ELSE ccPreClearStatus.CodeName
			END AS dis_grd_17377, --PreClearStatus,
	...
	BEGIN CATCH
		...
		SET @out_nReturnValue	=  @ERR_PRECLEARANCE_REQUEST_DETAILS_LIST_INSIDER
	END CATCH
