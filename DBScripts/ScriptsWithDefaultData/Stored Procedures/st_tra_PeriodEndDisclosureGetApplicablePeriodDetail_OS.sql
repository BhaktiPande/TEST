IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_PeriodEndDisclosureGetApplicablePeriodDetail_OS')
DROP PROCEDURE [dbo].[st_tra_PeriodEndDisclosureGetApplicablePeriodDetail_OS]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[st_tra_PeriodEndDisclosureGetApplicablePeriodDetail_OS]
	@inp_iUserInfoId 		INT,
...
BEGIN
	...
	BEGIN TRY
		...
		-- This will return flag to indicate if update tra_userperiodendmapping_OS table with new TP or old TP
		...
		SELECT @nPeriodType = CASE 
						 ...
						 END 
		FROM rul_TradingPolicy_OS TP WHERE TP.TradingPolicyId = @inp_nTradingPolicyId
		...
		IF(NOT EXISTS(SELECT UserPeriodEndMappingId FROM tra_UserPeriodEndMapping_OS WHERE UserInfoId = @inp_iUserInfoId))
		BEGIN
		...
		BEGIN
			...
			FROM tra_UserPeriodEndMapping_OS UPEM 
			...
			SELECT @nPeriodTypeOld = CASE 
								...
								ELSE TP.DiscloPeriodEndFreq 
							 END 
			FROM rul_TradingPolicy_OS TP WHERE TP.TradingPolicyId = @nTradingPolicyIdOld
			...
			BEGIN
				...
				BEGIN
					...
					IF NOT EXISTS(SELECT * FROM tra_TransactionMaster_OS TM 
				...
				BEGIN
					...
					IF (0 >= (SELECT COUNT(TM.TransactionMasterId) FROM tra_TransactionDetails_OS TD 
				...
				BEGIN
					...
					BEGIN
						...
						-- if PE is submitted then check for current period records exists and if not exists then add record as per pervious TP for current period 
						...
						BEGIN
							...
							SELECT @dtCurrPEEndDateOld = PEEndDate FROM tra_UserPeriodEndMapping_OS
