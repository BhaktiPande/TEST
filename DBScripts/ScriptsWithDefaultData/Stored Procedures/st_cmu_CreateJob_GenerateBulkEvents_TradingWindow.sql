...
IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_cmu_CreateJob_GenerateBulkEvents_TradingWindow')
DROP PROCEDURE [dbo].[st_cmu_CreateJob_GenerateBulkEvents_TradingWindow]
GO
SET ANSI_NULLS ON
GO
...
Description:	This procedure when executed creats a job which will run on daily basis. 
				The job created after exectuing this procedure, will run once everyday and execute procedure "st_cmu_Job_GenerateBulkEvents_TradingWindow" 
				to generate eventlog entries in bulk, into table "eve_EventLog" for events 'Trading Window Close (153025)' and 'Trading Window Open (153026)'
...
CREATE PROCEDURE st_cmu_CreateJob_GenerateBulkEvents_TradingWindow
(
...
BEGIN
	...
	DECLARE @ERR_JOB_CREATE_GENERATEBULKEVENTS_TRADINGWINDOW INT = -1

	BEGIN TRY
		--USE [msdb]
		/*--------- CREATE GenerateBulkEvents_TradingWindow JOB ---------------------*/
		DECLARE @ReturnCode INT
		SET @sJobName = 'KPCSTWBulkEveComp' + CAST(@inp_nMasterDBCompanyId AS VARCHAR(10)) + '_' + @sDBName
		...
		EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@sJobName, 
				...
				@notify_level_page=0, 
				@delete_level=0, 
				@description=N'Job to populate records to eventlog for trading window bulk events', 
		...
		EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, 
				...
				EXECUTE @RC = [dbo].[st_cmu_Job_GenerateBulkEvents_TradingWindow] 
				GO', 
		...
		SET @sScheduleName = N'KPCSTWBulkEveComp'+ CAST(@inp_nMasterDBCompanyId AS VARCHAR(10)) +'_Daily_Once'
		EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, 
				...
				@active_start_time=223000, --Start time is set as 10:30pm timing 22:30:00pm
	...
	BEGIN CATCH
		...
		RETURN @ERR_JOB_CREATE_GENERATEBULKEVENTS_TRADINGWINDOW
	END CATCH
END