IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_EmployeeInitialDisclosureList')
DROP PROCEDURE [dbo].[st_tra_EmployeeInitialDisclosureList]
GO
SET ANSI_NULLS ON
GO
...
GO
/*-------------------------------------------------------------------------------------------------
Description:	Procedure to get Initial Disclosure List for Co Own Securities for Employee
...
Usage:
EXEC st_tra_EmployeeInitialDisclosureList 
-------------------------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[st_tra_EmployeeInitialDisclosureList]
...
select TransactionMasterId from tra_TransactionMaster where UserInfoId=@inp_sUserInfoId 
...
CREATE TABLE #tUserData (Seq INT identity(1,1),UserInfoId INT,TMID INT, Name VARCHAR(max),Designation VARCHAR(max),
		...
		IF (@inp_sSortField  = 'dis_grd_52065' OR @inp_sSortField  = 'dis_grd_52066' OR @inp_sSortField  = 'dis_grd_50756' OR @inp_sSortField  = 'dis_grd_50757' OR @inp_sSortField  = 'dis_grd_50758')
		BEGIN 
		...
		SELECT 
			...
			CASE WHEN TU.UserTypeCodeId!=101007 THEN 'Self & Relative' ELSE 'Relative' END AS dis_grd_52065,			 
			DocumentUploadStatus as dis_grd_52066,
			DocumentUploadStatus as DocumentUploadStatus,
			tu.InitialDisclosureDate	 As dis_grd_50756,			
			...
			tu.SoftCopyDate				 As dis_grd_50757,
			tu.TextHardCopyDate			 As TextHardCopyDate,
			tu.HardCopyDate				 As dis_grd_50758,
