IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_tra_ExerciseBalancePoolUpdateDetails')
DROP PROCEDURE [dbo].[st_tra_ExerciseBalancePoolUpdateDetails]
GO
SET ANSI_NULLS ON
GO
...
Parag			03-Dec-2015		Made change to update exercise security pool when preclearance request is close
Parag			03-Dec-2015		Made change to add input parameter for procedure 
Parag			08-Dec-2015		Made change to use decimal instead of int for quantity field
Parag			11-Dec-2015		Made change to fix issue found during testing 
Tushar			09-Mar-2016		Change related to Selection of QTY Yes/No configuration. 
								(Based on contra trade functionality)
Parag			13-Jun-2016		Made change to maintain security pool for all security type
...
Tushar			08-Sep-2016		Declare cursor first then initialize	
								
Usage:
EXEC st_tra_ExerciseBalancePoolUpdateDetails <user id> <security type>
-------------------------------------------------------------------------------------------------*/
CREATE PROCEDURE [dbo].[st_tra_ExerciseBalancePoolUpdateDetails]
...
BEGIN
	...
	DECLARE @nPreclearanceQuantityOld DECIMAL = 0
	...
	BEGIN TRY
		...
		LEFT JOIN tra_PreclearanceRequest PCL ON TM.PreclearanceRequestId = PCL.PreclearanceRequestId
		WHERE 
			...
			IF(@nDMATDetailsID IS NULL OR @nDMATDetailsID='')
			BEGIN			
				SELECT @nDMATDetailsID=DMATDetailsID FROM tra_TransactionDetails TD JOIN tra_TransactionMaster TM
				ON TD.TransactionMasterId=TM.TransactionMasterId
				WHERE TM.TransactionMasterId=@inp_nTransactionMasterId AND TD.DMATDetailsID IS NOT NULL	
			END	
		...
		--		@nPreclearanceStatusCodeId as '@nPreclearanceStatusCodeId',  @nESOPExcerciseOptionQtyFlag as '@nESOPExcerciseOptionQtyFlag', 
		--		@nOtherESOPExcerciseOptionQtyFlag as '@nOtherESOPExcerciseOptionQtyFlag', 
		...
		BEGIN
			...
			SELECT @nESOPQuanity_FromPool = ESOPQuantity, @nOtherQuantity_FromPool = OtherQuantity, @nPledgeQuantity_FromPool = isnull(PledgeQuantity,0), @nNotImpactedQuantity_FromPool = isnull(NotImpactedQuantity,0) FROM tra_ExerciseBalancePool 
			...
			--		+ ' @nPCLOtherExcerciseOptionQty ' +convert(varchar, ISNULL(@nOtherExcerciseOptionQty,-1))
			 ...
			 SELECT @nPreclearanceQuantityOld = SecuritiesToBeTradedQtyOld FROM tra_PreclearanceRequest PR 	
		     WHERE PR.UserInfoId = @nUserInfoId AND PR.PreclearanceRequestId = @nPreClearanceRequestId
			IF (@inp_nMapToTypeCodeId = @nAction_PreclearanceRequest)
			BEGIN
				print 'Preclearance Request - '
				
			  IF (@nPreclearanceStatusCodeId = @nPreClearanceStatus_Approve AND @nIsPreClearanceEditable = 524001)
			  BEGIN
					IF (@nTransactionTypeCodeId = @nTransactionType_Buy)
					BEGIN						    
						SET @nOtherQuantity = @nPreclearanceQuantityOld - @nPCLSecuritiesToBeTradedQty

						UPDATE tra_ExerciseBalancePool SET OtherQuantity = OtherQuantity - @nOtherQuantity WHERE UserInfoId = @nUserInfoId and SecurityTypeCodeId = @nPCLSecurityTypeCodeId and DMATDetailsID = @nDMATDetailsID
					END
				...
				END
				ELSE IF (@nTransactionTypeCodeId = @nTransactionType_CashExercies)
				BEGIN
				--PRINT '@nContraTradeQuantityBase' + Convert(varchar(max),@nContraTradeOption)
				IF(@nContraTradeOption = @nContraTradeQuantityBase)
				BEGIN
					-- cash exercise transaction affect ESOP pool quantity only
				...
				END
				END
				ELSE IF (@nTransactionTypeCodeId = @nTransactionType_CashlessPartail)
				BEGIN
			
				IF(@nContraTradeOption = @nContraTradeQuantityBase)
				BEGIN
					-- cashless partial transaction affect ESOP pool quantity only
					SET @nESOPQuanity = @nESOPQuanity_FromPool + CASE 
											WHEN @nPreclearanceStatusCodeId = @nPreClearanceStatus_Reject THEN -@nPCLESOPExcerciseOptionQty 
											ELSE @nPCLESOPExcerciseOptionQty END
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						SELECT 
							@nTradedQuantity = SUM(td.Quantity), @nTradedQuantity2 = SUM(td.Quantity2), 
							@nTradedESOPExcerciseOptionQty = SUM(td.ESOPExcerciseOptionQty), 
							@nTradedOtherExcerciseOptionQty = SUM(td.OtherExcerciseOptionQty)
						...
						BEGIN
							...
							BEGIN
								...
								END
							END
							ELSE IF (@nPCLSecuritiesToBeTradedQty = @nTradedQuantity AND @nTransactionTypeCodeId = @nTransactionType_CashlessPartail)
							BEGIN
								...
								SET @nDifferenceESOPQuantity = 0
								SET @nDifferenceOtherQuantity = 0
							END
							...
							BEGIN
								...
								BEGIN
									...
									BEGIN
										-- cashless partial transaction affect ESOP pool quantity only
										SET @nESOPQuanity = @nESOPQuanity_FromPool + CASE 
														WHEN @nIsUnderOROverTraded = 1 THEN -@nDifferenceQuantity - @nTradedQuantity2 -- under trade
														WHEN @nIsUnderOROverTraded = 2 THEN @nDifferenceQuantity - @nTradedQuantity2 -- over trade
														WHEN @nIsUnderOROverTraded = 3 THEN @nDifferenceQuantity - @nTradedQuantity2 -- same trade
														END
				...
				BEGIN
					...
					BEGIN
						...
						SET @TraDetail_Cursor = CURSOR FOR 
							SELECT 
								TD.TransactionDetailsId, TD.SecurityTypeCodeId, TD.TransactionTypeCodeId, TD.Quantity, TD.Quantity2, 
								ESOPExcerseOptionQtyFlag, OtherESOPExcerseOptionFlag, ESOPExcerciseOptionQty, OtherExcerciseOptionQty, ModeOfAcquisitionCodeId, TD.LotSize,TD.DMATDetailsID
							FROM tra_TransactionDetails TD JOIN 
							tra_TransactionMaster TM ON TD.TransactionMasterId = TM.TransactionMasterId 
								AND TM.TransactionMasterId = @inp_nTransactionMasterId AND TD.DMATDetailsID IS NOT NULL
						...
						BEGIN
							...
							BEGIN
								...
								BEGIN
									-- cash exercise transaction affect ESOP pool quantity only									
									SET @nESOPQuanity = @nQuantityForESOP
									-- set quantity from pool
									SET @nOtherQuantity = @nOtherQuantity
									SET @nPledgeQuantity = @nPledgeQuantity
							...
							BEGIN
								...
								BEGIN
									...
									BEGIN
										-- requested quantity is less than ESOP quantity in pool
										IF (@nQuantity <= @nESOPQuanity_FromPool)
										BEGIN											
											SET @nESOPQuanity = @nQuantityForESOP
									...
									BEGIN
										-- requested quantity is less than Others quantity in pool
										IF (@nQuantity <= @nOtherQuantity_FromPool)
										BEGIN
											SET @nOtherQuantity = @nQuantityForOther
											SET @nPledgeQuantity = @nPledgeQuantity
											SET @nNotImpactedQuantity = @nQuantityForNotImpacted															
							...
							/*	INSERT INTO tra_ExerciseBalancePool 
									(UserInfoId, SecurityTypeCodeId, ESOPQuantity, OtherQuantity, PledgeQuantity, NotImpactedQuantity)
								VALUES
									(@nUserInfoId, @nTDSecurityTypeCodeID, @nESOPQuanity, @nOtherQuantity, @nPledgeQuantity, @nNotImpactedQuantity)
							END
							ELSE IF @nPoolActionFlag = 2 -- update existing record
							BEGIN
								UPDATE tra_ExerciseBalancePool SET 
									ESOPQuantity = @nESOPQuanity, 
									OtherQuantity = @nOtherQuantity,
									PledgeQuantity = @nPledgeQuantity,
									NotImpactedQuantity = @nNotImpactedQuantity
								WHERE UserInfoId = @nUserInfoId AND SecurityTypeCodeId = @nTDSecurityTypeCodeID
							END
							*/
							...
							--select @nUserInfoId AS '@nUserInfoId',@nDematDetailsID, @nTDSecurityTypeCodeID, @nESOPQuanity,@nOtherQuantity,@nPledgeQuantity,@nNotImpactedQuantity
								...
								EXEC @nTmpRet = st_tra_UpdateESOPPoolDematwise @nUserInfoId,@nDematDetailsID, @nTDSecurityTypeCodeID, @nESOPQuanity,@nOtherQuantity,@nPledgeQuantity,@nNotImpactedQuantity,
			...
			--print '@bChangePoolFlag '+convert(varchar, @bChangePoolFlag)+' @nPoolActionFlag '+convert(varchar, @nPoolActionFlag)
			--		+' @nUserInfoId '+convert(varchar, @nUserInfoId)+' @nSecurityType_Share '+convert(varchar, @nSecurityType_Share)
			--		+' @nESOPQuanity '+convert(varchar, ISNULL(@nESOPQuanity,-1))+' @nOtherQuantity '+convert(varchar, ISNULL(@nOtherQuantity,-1))
			...
			BEGIN
				...
				--		(UserInfoId, SecurityTypeCodeId, ESOPQuantity, OtherQuantity, PledgeQuantity, NotImpactedQuantity)
				--	VALUES
				--		(@nUserInfoId, @nTransaction_SecurityType, @nESOPQuanity, @nOtherQuantity, @nPledgeQuantity, @nNotImpactedQuantity)
				--END
				--ELSE IF @nPoolActionFlag = 2 -- update existing record
				--BEGIN
				--	UPDATE tra_ExerciseBalancePool SET 
				...
				--	WHERE UserInfoId = @nUserInfoId AND SecurityTypeCodeId = @nTransaction_SecurityType
				
				IF(@nDMATDetailsID IS NOT NULL AND @nDMATDetailsID <>'')
				BEGIN
					EXEC @nTmpRet = st_tra_UpdateESOPPoolDematwise @nUserInfoId,@nDMATDetailsID, @nTransaction_SecurityType, @nESOPQuanity,@nOtherQuantity,@nPledgeQuantity,@nNotImpactedQuantity,
												@out_nReturnValue OUTPUT, @out_nSQLErrCode OUTPUT, @out_sSQLErrMessage OUTPUT
