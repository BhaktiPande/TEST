IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_rul_ApplicabilityFilterList_Employee')
DROP PROCEDURE [dbo].[st_rul_ApplicabilityFilterList_Employee]
GO
SET ANSI_NULLS ON
GO
...
/*-------------------------------------------------------------------------------------------------
Description:	Procedure to list Applicability association filter sets used during applicability allocation - Employee.
...
Gaurishankar	12-Apr-2016			Changes for AllCo, AllCorporateInsider, AllNonEmployeeInsider and Non Insider Employee Applicability.
Raghvendra		07-Sep-2016			Changed the GETDATE() call with function dbo.uf_com_GetServerDate(). This function will return the date to be used as server date.
									Added new filter Category, SubCategory & Role for applicability.
Gaurishankar	14-Oct-2016			fixed issue for Role Name not displyed in Filter list
Usage:
EXEC st_rul_ApplicabilityFilterList_Employee 10, 1, '','ASC', 132001, 2,
-------------------------------------------------------------------------------------------------*/

CREATE PROCEDURE [dbo].[st_rul_ApplicabilityFilterList_Employee]
	@inp_iPageSize			INT = 10
	,@inp_iPageNo			INT = 1
	,@inp_sSortField		VARCHAR(255)
	,@inp_sSortOrder		VARCHAR(5)
	,@inp_nGridTypeCodeId	INT		-- 114032 for Employee Insider , 114090 for Employee non insider.
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			Select @nAppMaxVersionNumber = VersionNumber from rul_ApplicabilityMaster where ApplicabilityId= @inp_nApplicabilityId
		END
		ELSE
		BEGIN
		...
		IF (@inp_nGridTypeCodeId = 114032 )
		BEGIN 
			SELECT 
			ApplicabilityDtlsId, 
		    --CdUIDepartment.CodeName,CdUIGrade.CodeName, CdUIDesignation.CodeName,
			  ISNULL((CASE WHEN CdUIDepartment.DisplayCode IS NULL OR CdUIDepartment.DisplayCode = '' THEN CdUIDepartment.CodeName ELSE CdUIDepartment.DisplayCode END),'') AS rul_grd_15339,--DepartmentName,
			  ISNULL((CASE WHEN CdUIGrade.DisplayCode IS NULL OR CdUIGrade.DisplayCode = '' THEN CdUIGrade.CodeName ELSE CdUIGrade.DisplayCode END),'') AS rul_grd_15340,--GradeName,
			  ISNULL((CASE WHEN CdUIDesignation.DisplayCode IS NULL OR CdUIDesignation.DisplayCode = '' THEN CdUIDesignation.CodeName ELSE CdUIDesignation.DisplayCode END ),'') AS rul_grd_15341,--DesignationName,
			  ...
			  ISNULL((CASE WHEN CdUICategory.DisplayCode IS NULL OR CdUICategory.DisplayCode = '' THEN CdUICategory.CodeName ELSE CdUICategory.DisplayCode END ),'') AS rul_grd_15451,--Category
			  ISNULL((CASE WHEN CdUISubCategory.DisplayCode IS NULL OR CdUISubCategory.DisplayCode = '' THEN CdUISubCategory.CodeName ELSE CdUISubCategory.DisplayCode END ),'') AS rul_grd_15452,--Sub Category
			  UR.RoleName AS rul_grd_15453, -- Role	
			  ...
			  INNER JOIN rul_ApplicabilityMaster AM ON AD.ApplicabilityMstId = AM.ApplicabilityId
