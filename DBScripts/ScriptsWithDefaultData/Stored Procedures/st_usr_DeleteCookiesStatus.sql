...
IF EXISTS(SELECT NAME FROM SYS.PROCEDURES WHERE NAME = 'st_usr_DeleteCookiesStatus')
	DROP PROCEDURE st_usr_DeleteCookiesStatus
GO

CREATE PROCEDURE st_usr_DeleteCookiesStatus --0,'Unauthorised','6ED9279A-D0BA-4DE1-B323-D6FD93A9DC8A~MONDAY',0,0
	@inp_UserId				INT=0,
	@inp_CookieStatus		varchar(100)=NULL,
...
BEGIN TRY
	...
	BEGIN
		...
		DELETE FROM usr_UserWiseFormTokenList WHERE UserInfoID=@inp_UserId
	END
	ELSE
	BEGIN
		UPDATE usr_UserCookies SET CookieName='Unauthorised' WHERE UserInfoId = @inp_UserId  
	END	
